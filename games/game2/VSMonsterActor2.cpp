...
void VSMonsterActor2::CreateDefaultComponentNode()
{
	...
	VSSkeletonMeshNodeRPtr  pResource = VSResourceManager::LoadASYNResource<VSSkeletonMeshNode>(_T("AnimTreeMonster2.SKMODEL"), false);
	
	m_pMonsterMC->SetSkeletonMeshResource(pResource);
	
	m_pThirdCamera = AddComponentNode<VSCamera>();
	Vector3 CameraPos(0.0f, 400.0f, 300.0f);
	Vector3 CameraDir(0.0f, -1.0f, -1.0f);
	...
	m_pViewFamily = MEM_NEW VSWindowViewFamily(_T("WindowUse"), m_pThirdCamera, NULL, VSForwardEffectSceneRenderMethod::ms_Type.GetName().GetBuffer(), -1);

}
void VSMonsterActor2::Update(double dAppTime)
{
	Matrix3X3 Rotator = GetWorldRotate();
	Vector3 U, V, N;
	Rotator.GetUVN(U, V, N);
	Vector3 FrontDir = N * (-1.0f);

	Vector3 Pos = GetWorldPos();
	Pos += FrontDir * m_fMoveSpeed * (float)Timer::ms_pTimer->GetDetTime();
	SetWorldPos(Pos);

	if (m_bLMouseDowning)
	{
		m_RotYDelta = ((m_iDetMouseX)* 1.0f) * (float)Timer::ms_pTimer->GetDetTime();
		m_iDetMouseX = 0;
...
{
	float ParaFinal = 0.0f;
	float ParaIdle_Walk = 0.0f;
	...
	if (m_bAttack == true)
	{
		...
		m_fAttackStartTime += (float)Timer::ms_pTimer->GetDetTime();

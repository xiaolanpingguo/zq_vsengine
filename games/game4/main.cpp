...
bool VSDemoWindowsApplication::OnInitial()
{
	...
	m_pCameraActor->GetTypeNode()->AddController(m_p1stCameraController);
 	Vector3 CameraPos(0.0f, 0.0f, -300.0f);
	...
	for (unsigned int y = 0; y < 1; y++)
	{
		for (int z = -RENDER_NUM; z <= RENDER_NUM; z++)
		{
			...
			{
				VSWorld::ms_pWorld->CreateActor(_T("NewStone.STMODEL"),Vector3(x * 400.0f,y * 400.0f,z * 400.0f),Matrix3X3::ms_Identity, Vector3::ms_One, m_pTestMap);
			}
		}

...
char Image::ms_ImageFormat[IF_MAX][10] = 
{
	_T("bmp"),
	_T("tga")
};
...
bool VSBMPImage::LoadFromBuffer(unsigned char * pBuffer,uint32 uiSize)
{
	if (!pBuffer)
	{
		return false;
...
bool VSBMPImage::Load(const char* pFilename)
{
...
bool VSBMPImage::GetFile(const char * pFilename)
{
	File ImageFile;
	ImageFile.Open(pFilename,File::OM_RB);

	VSMAC_ASSERT(ImageFile.GetFileSize());
	m_pData = MEM_NEW unsigned char[ImageFile.GetFileSize()];
	if (!m_pData)
	{
		return false;
	}
...
bool VSBMPImage::LoadBmpRaw()
{
	...
	m_pImageData = MEM_NEW unsigned char[m_uiDataSize];

	VSMAC_ASSERT(m_pImageData);

	VSMemcpy(m_pImageData,&m_pData[uiImgOffset],m_uiDataSize);
...
bool VSBMPImage::LoadBmpRLE8()
{
	...
	unsigned char *pSrc;
	uint32 iDcode=1,iCount,iPos,iIndex;
...
bool VSBMPImage::LoadBmpPalette()
{
	...
	m_pPalette = MEM_NEW unsigned char[768];
	VSMAC_ASSERT(m_pPalette);
...
const unsigned char *VSBMPImage::GetPixel(uint32 x, uint32 y)const
{
	if ( x < 0 ) x = 0;
	...
	int32 bytes_per_pixel = m_uiBPP / 8;
	int32 loc = (y * m_uiWidth + x) * bytes_per_pixel;
...
bool VSTGAImage::LoadFromBuffer(unsigned char * pBuffer,uint32 uiSize)
{	
...
bool VSTGAImage::Load(const char* pFilename)
{
	...
	File ImageFile;
	if(!ImageFile.Open(pFilename,File::OM_RB))
		return false;
	...
	m_pData = MEM_NEW unsigned char[ulSize];
...
bool VSTGAImage::LoadRawData() // Load uncompressed image data
{
	...
	VSMemcpy(m_pImageData,&m_pData[iOffset],m_uiDataSize);
	...
	while(Index < m_uiDataSize) 
	{
		if(*pCur & 0x80) // Run length chunk (High bit = 1)
		{
			...
			for(bLoop = 0 ; bLoop != bLength; ++bLoop,Index += iPixelSize)
				VSMemcpy(&m_pImageData[Index],pCur,iPixelSize);
...
const unsigned char *VSTGAImage::GetPixel(uint32 x, uint32 y)const
{
	if ( x < 0 ) x = 0;
	if ( x >= m_uiWidth ) x = m_uiWidth-1;
	if ( y < 0 ) y = 0;

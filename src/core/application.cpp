...
#include "graphic/asyn/monitor.h"
...
DECLEAR_TIME_PROFILENODE(Present, Update)
VSApplication *VSApplication::ms_pApplication = NULL;
VSApplication::VSApplication()
{
	m_uiRenderAPIType = VSRenderer::RAT_NULL;
...
bool VSApplication::CreateTimer()
{
	if (!Timer::ms_pTimer)
	{
		Timer * pTimer = MEM_NEW Timer();
		if (!pTimer)
		{
			return false;
		}
	}
	VSRandInit(Timer::ms_pTimer->GetRandSeed());
	return true;
}
bool VSApplication::CreateSceneManager()
{
	...
	{
		VSSceneManager * pSceneManager = MEM_NEW VSSceneManager();
		if (!pSceneManager)
		{
			return false;
		}
...
bool VSApplication::CreateStreamingManager()
{
	if (!VSStreamingManager::ms_pStreamingManager)
	{
		VSStreamingManager* pStreamingManager = MEM_NEW VSStreamingManager();
		if (!pStreamingManager)
		{
			return false;
		}
	}
...
bool VSApplication::CreateWorld()
{
	...
	{
		VSWorld * pWorld = MEM_NEW VSWorld();
		if (!pWorld)
		{
			return false;
		}
...
bool VSApplication::CreateASYNLoader()
{
	...
	{
		VSASYNLoadManager * pASYNLoadManager = MEM_NEW VSASYNLoadManager();
		if (!pASYNLoadManager)
		{
			return false;
		}
...
bool VSApplication::CreateUpdateThread()
{
	...
	{
		VSUpdateThreadSys * pUpdateThreadSys = MEM_NEW VSUpdateThreadSys(m_iUpdateThreadNum);
		if (!pUpdateThreadSys)
		{
			return false;
		}
...
bool VSApplication::CreateRenderThread()
{
	...
	{
		VSRenderThreadSys * pRenderTreadSys = MEM_NEW VSRenderThreadSys();
		if (!pRenderTreadSys)
		{
			return false;
		}
...
{
	SAFE_DELETE(VSRenderer::ms_pRenderer);
	return true;
}
bool VSApplication::ReleaseInput()
{
	SAFE_DELETE(VSEngineInput::ms_pInput);
	return true;
}
bool VSApplication::ReleaseTimer()
{
	SAFE_DELETE(Timer::ms_pTimer);
	return true;
}
bool VSApplication::ReleaseSceneManager()
{
	SAFE_DELETE(VSSceneManager::ms_pSceneManager);
	return true;
}
bool VSApplication::ReleaseStreamingManager()
{
	SAFE_DELETE(VSStreamingManager::ms_pStreamingManager);
	return true;
}
bool VSApplication::ReleaseWorld()
{
	SAFE_DELETE(VSWorld::ms_pWorld);
	return true;
}
bool VSApplication::ReleaseASYNLoader()
{
	SAFE_DELETE(VSASYNLoadManager::ms_pASYNLoadManager);
	return true;
}
bool VSApplication::ReleaseRenderThread()
{
	...
	SAFE_DELETE(VSRenderThreadSys::ms_pRenderThreadSys);
	return true;
}
bool VSApplication::ReleaseUpdateThread()
{
	...
	SAFE_DELETE(VSUpdateThreadSys::ms_pUpdateThreadSys);
	return true;
}
...
bool VSApplication::Run()
{
	...
	if (Timer::ms_pTimer)
	{
		...
		fTime = Timer::ms_pTimer->GetGamePlayTime();
		fFPS = Timer::ms_pTimer->GetFPS();
		ADD_ONLYTIME_PROFILE(FPS, (float)fFPS)
	}
	if (VSRenderer::ms_pRenderer)
	{
		...
		if(VSRenderer::ms_pRenderer->CooperativeLevel())
		{
			...
			}			
			{
				...
				VSCullerManager::GetCullerManager().FillGeometryOcclusionQueryData();
				...
				if (VSStreamingManager::ms_pStreamingManager)
				{
					VSStreamingManager::ms_pStreamingManager->Update(fTime);
				}
		...
		else
		{
			...
			if (VSASYNLoadManager::ms_pASYNLoadManager)
			{
				VSASYNLoadManager::ms_pASYNLoadManager->FlushAllJob();
			}
			if (VSStreamingManager::ms_pStreamingManager)
			{
				VSStreamingManager::ms_pStreamingManager->ReleaseAllStream();
			}
			VSResourceManager::GC();
		}
		VSResourceManager::DelayUpdate((float)fTime);
...
bool VSApplication::Main(HINSTANCE hInst, LPSTR lpCmdLine, int32 nCmdShow)
{
	return false;
}
bool VSApplication::Main(int32 argc, char* argv[])
{
	return false;
}
...
bool VSApplication::PreInitial()
{
	...
	int32 iUpdateMaxThreadNum = VSGetCpuNum();
	iUpdateMaxThreadNum--;// Main Thread all time run
	iUpdateMaxThreadNum -= 2;// AsynLoad 2 Thread ,Monitor Thread
...
void VSApplication::ChangeScreenSize(uint32 uiWidth,uint32 uiHeight,bool bWindow,bool IsMaxScreen)
{
	if (VSRenderThreadSys::ms_pRenderThreadSys)
	{
		VSRenderThreadSys::ms_pRenderThreadSys->Clear();
...
void VSApplication::OnMove(int32 xPos,int32 yPos)
{

}
void VSApplication::OnReSize(int32 iWidth,int32 iHeight)
{

}
void VSApplication::OnKeyDown(uint32 uiKey)
{

}
void VSApplication::OnKeyUp(uint32 uiKey)
{

}
void VSApplication::OnLButtonDown(int32 xPos,int32 yPos)
{

}
void VSApplication::OnLButtonUp(int32 xPos,int32 yPos)
{

}
void VSApplication::OnRButtonDown(int32 xPos,int32 yPos)
{

}
void VSApplication::OnRButtonUp(int32 xPos,int32 yPos)
{

}
void VSApplication::OnMButtonDown(int32 xPos,int32 yPos)
{

}
void VSApplication::OnMButtonUp(int32 xPos,int32 yPos)
{

}
void VSApplication::OnMouseMove(int32 xPos,int32 yPos)
{

}
void VSApplication::OnMouseWheel(int32 xPos,int32 yPos,int32 zDet)
{

}
#ifdef PLATFORM_WIN
uint32 VSWindowApplication::ms_WindowKeyToVS[] =
{
	...
	if (VSRenderer::ms_pRenderer)
	{
		...
		else
		{
			...
			VSDX11Renderer * pRenderer =
				MEM_NEW VSDX11Renderer(m_MainHwnd, m_uiScreenWidth, m_uiScreenHeight,
...
bool VSWindowApplication::CreateRenderer()
{
	...
	if (m_uiRenderAPIType == VSRenderer::RAT_DIRECTX11)
	{
		return CreateDx11();
	}
	else
...
bool VSWindowApplication::CreateInput()
{
	if (VSEngineInput::ms_pInput)
	{
		...
		else
		{
			...
			{
				VSEngineInput * pInput = MEM_NEW VSEngineDXInput(m_hInst, m_MainHwnd, NULL);
				if (!pInput)
				{
					return false;
				}
...
bool VSWindowApplication::Main(HINSTANCE hInst, LPSTR lpCmdLine, int32 nCmdShow)
{
	m_pCommand = MEM_NEW VSCommand(lpCmdLine);
	m_hInst = hInst;
	m_bIsRunning = true;
	m_bIsActive = false;
	if (!CreateTimer())
...
bool VSWindowApplication::PreInitial()
{
	...
	m_uiAnisotropy = 1;
	m_uiMultisample = VSRenderer::MS_NONE;
	m_Tile = _T("zq.0 by azhe --- Demo ") + VSRenderer::GetRenderTypeString(m_uiRenderAPIType);
#ifdef _WIN64
	m_Tile += _T(" x64");
#else
	m_Tile += _T(" x86");
...
bool VSWindowApplication::CreateAppWindow()
{
	...
	if (m_bIsWindowed)
	{
		RECT window_rect = { 0, 0, (LONG)m_uiScreenWidth, (LONG)m_uiScreenHeight };
		AdjustWindowRect(&window_rect, m_dwWindowedStyle, GetMenu(m_MainHwnd) != NULL);
		int32 NewWidth = window_rect.right - window_rect.left;
		int32 NewHeight = window_rect.bottom - window_rect.top;
		//移动窗口
...
void VSWindowApplication::ChangeScreenSize(uint32 uiWidth, uint32 uiHeight, bool bWindow, bool IsMaxScreen)
{
	...
	if (bWindow)
	{
		...
		else
		{
			...
			uint32 NewWidth = window_rect.right - window_rect.left;
			uint32 NewHeight = window_rect.bottom - window_rect.top;
			//移动窗口
			MoveWindow(m_MainHwnd,
				0, // x position
				0, // y position
...
void VSApplication::InputMsgProc(uint32 uiInputType, uint32 uiEvent, uint32 uiKey, int32 x, int32 y, int32 z)
{
...
uint32 VSWindowApplication::CheckVirtualKeyDown(uint32 VK)
{
...
uint32 VSWindowApplication::CheckVirtualKeyUp(uint32 VK)
{
	if (GetKeyState(VK_LSHIFT) > 0)
	{
		return VK_LSHIFT;
...
LRESULT WINAPI VSWindowApplication::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		...
		case WM_MOVE:
		{
			int32 xPos = (int)(LOWORD(lParam));
			int32 yPos = (int)(HIWORD(lParam));
		...
		case WM_KEYDOWN:
		{
			if (!VSEngineInput::ms_pInput)
			{
				uint32 virtKey = (unsigned int)wParam;
				if (virtKey < sizeof(ms_WindowKeyToVS) / sizeof(DWORD))
				{ 
					VSApplication::ms_pApplication->InputMsgProc(VSEngineInput::IT_KEYBOARD, VSEngineInput::IE_DOWN, ms_WindowKeyToVS[virtKey],0,0,0);
				}
		...
		case WM_KEYUP:
		{
			if (!VSEngineInput::ms_pInput)
			{
				...
				if (virtKey < sizeof(ms_WindowKeyToVS) / sizeof(DWORD))
				{
					VSApplication::ms_pApplication->InputMsgProc(VSEngineInput::IT_KEYBOARD, VSEngineInput::IE_UP, ms_WindowKeyToVS[virtKey], 0, 0, 0);
				}
...
bool VSWindowApplication::CreateMonitor()
{
	if (!VSResourceMonitor::ms_pResourceMonitor)
	{
		VSResourceMonitor * pResourceMonitor = MEM_NEW VSResourceMonitor();
		if (!pResourceMonitor)
		{
			return false;
		}
...
bool VSWindowApplication::ReleaseMonitor()
{
	...
	SAFE_DELETE(VSResourceMonitor::ms_pResourceMonitor);
	return true;
}
...
bool VSConsoleApplication::CreateDx11()
{
	if (!VSRenderer::ms_pRenderer)
	{
		VSDX11Renderer * pRenderer =
			MEM_NEW VSDX11Renderer(NULL, 0, 0,
				false, m_uiAnisotropy, m_uiMultisample, NULL,0);
		if (VSRenderer::ms_pRenderer)
		{
			return true;
		}
...
bool VSConsoleApplication::CreateRenderer()
{
	...
	else if (m_uiRenderAPIType == VSRenderer::RAT_NULL)
	{
		if (!VSRenderer::ms_pRenderer)
		{
			VSNullRenderer * pRenderer = MEM_NEW VSNullRenderer();
			if (VSRenderer::ms_pRenderer)
			{
				return true;
			}
...
bool VSConsoleApplication::Main(int32 argc, char* argv[])
{
	m_pCommand = MEM_NEW VSCommand(argc, argv);
	m_bIsRunning = true;
	m_bIsActive = false;
	if (!CreateTimer())
	{
...
void VSConsoleApplication::ChangeScreenSize(uint32 uiWidth, uint32 uiHeight, bool bWindow, bool IsMaxScreen)
{
	VSApplication::ChangeScreenSize(uiWidth, uiHeight, bWindow, IsMaxScreen);
}

...

{
	...
	m_ZBias = 0.001f;
	m_LightFunScale = Vector2(0.001f,0.001f);
	m_LightFunOffset = Vector2(0.0f,0.0f);
...
{	
	...
	m_WorldRenderBV = Sphere(Point3, m_Range);
...
void VSPointLight::SetShadowType(uint32 uiShadowType)
{
	...
	else if (uiShadowType == ST_CUB)
	{
		...
		m_pShadowTexture[0]->SetTexture(MEM_NEW VSCubeTexture(m_uiRTWidth,VSRenderer::SFT_R16F, 1, true));
		...
		m_pShadowMapSceneRender = MEM_NEW VSShadowMapSceneRender(VSShadowMapSceneRender::SMT_CUB);
	...
	else if (uiShadowType == ST_DUAL_PARABOLOID)
	{
		...
		m_pShadowMapSceneRender = MEM_NEW VSShadowMapSceneRender(VSShadowMapSceneRender::SMT_DUAL_PARABOLOID);
		m_pShadowMapSceneRender->m_pLocalLight = this;
		m_pShadowMapSceneRender->SetParam(VSRenderer::CF_USE_ALL, VSColorRGBA(1.0f, 1.0f, 1.0f, 1.0f), 1.0f, 0);
	}
...
void VSPointLight::DrawNormalProjectShadow(VSCuller & CurCuller,double dAppTime)
{
	...
	if (m_pScene.GetNum() > 0)
	{
		for (uint32 Index = 0 ; Index < VSCubeTexture::F_MAX ;Index++)
		{
			...
			TempCamera.SetPerspectiveFov(MATH_PI2, 1.0f, 1.0f, m_Range);
...
void VSPointLight::DrawFovProjectShadow(VSCuller & CurCuller,double dAppTime)
{
	...
	Plane CameraPlane[VSCamera::CP_MAX];
	CurCuller.GetCamera()->GetPlane(CameraPlane);
	uint32 CullFlag = 0;
	for (uint32 i = 0 ; i < VSCamera::CP_MAX ; i++)
	{
		int32 iSide = WorldPos.RelationWith(CameraPlane[i]);
		if (iSide == MATH_BACK)
		{
...
void VSPointLight::DrawDualParaboloidShadow(VSCuller & CurCuller, double dAppTime)
{
	...
	if (m_pScene.GetNum() > 0)
	{
		...
		{
			float ZDirection = (Index == 0) ? 1.0f : -1.0f;
			DPCamera[Index].CreateFromLookDir(WorldPos, Vector3(0.0f, 0.0f, ZDirection));

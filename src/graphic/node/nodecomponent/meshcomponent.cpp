...
void VSMeshComponent::ComputeNodeVisibleSet(VSCuller & Culler, bool bNoCull, double dAppTime)
{
	...
	if (m_pNode)
	{
		...
		for (uint32 i = 0; i < m_UseID.GetNum(); i++)
		{
...
VSMaterialInstance * VSMeshComponent::GetUseMaterialInstance(uint32 SubMeshID, uint32 LOD)const
{
...
VSMaterialInstance * VSMeshComponent::GetMaterialInstance(uint32 SubMeshID, uint32 UseID, uint32 LOD)const
{
...
bool VSMeshComponent::SetUseMaterialInstance(uint32 SubMeshID, uint32 UseID)
{
	if (SubMeshID >= m_UseID.GetNum())
	{
		return false;
...
uint32 VSMeshComponent::GetSubMeshNum()
{
	VSGeometryNode * pGeometryNode = m_pNode->GetGeometryNode(0);

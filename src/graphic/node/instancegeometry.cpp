...
VSInstanceGeometry::~VSInstanceGeometry()
{
	SAFE_DELETEA(m_pInstanceDataBuffer);
	SAFE_DELETEA(m_pInstanceDataRenderBuffer);
...
{	
	...
	{
		for (uint32 i = 0 ; i < VSResourceManager::GetSimpleInstanceAnimFloat4Num(); i++)
		{
...
void VSInstanceGeometry::SetMaxInstanceBuffer(VSArray<VSVertexFormat::VERTEXFORMAT_TYPE> &FormatArray,uint32 uiMaxNum)
{
	m_uiMaxInstanceNum = uiMaxNum;
	m_PendingDeleteInstanceBuffer = m_InstanceBuffer;
	
	m_InstanceBuffer = MEM_NEW VSVertexBuffer(FormatArray, m_uiMaxInstanceNum);
	m_InstanceBuffer->SetStatic(false);
	unsigned char * pInstanceDataBuffer = MEM_NEW unsigned char[m_uiMaxInstanceNum * m_uiInstanceSize];
...
void VSInstanceGeometry::ClearInstanceData()
{
	if (VSResourceManager::ms_bRenderThread)
	{
		...
		{
			SAFE_DELETEA(m_pInstanceDataRenderBuffer);
			m_pInstanceDataRenderBuffer = MEM_NEW unsigned char[m_uiMaxInstanceNum * m_uiInstanceSize];
			m_bReCreate = false;
		}
...
unsigned char * VSInstanceGeometry::GetCurDataBufferAddr(uint32 i)
{
	return m_pInstanceDataBuffer + i * m_uiInstanceSize;
}
uint32 VSInstanceGeometry::AddInstance(const AABB WorldBound, const VSTransform & T,VSMaterialInstance * pMaterialInstance, VSArray<ANIM_INSTANCE_SHADER_DATA> * pAnimData)
{
	...
	Vector3W * pData = (Vector3W *)GetCurDataBufferAddr(m_uiInstanceNum);
	...
	*pData = ColumnVector[2];
	...
	if (HaveSkinInstance())
	{
		...
		float * uiAnimNumPtr = (float *)(pData + VSResourceManager::GetSimpleInstanceAnimFloat4Num());
		uiAnimNumPtr--;
		...
		else
		{
			...
			for (uint32 i = 1; i < MAX_SIMPLE_INSTANCE_ANIM; i++)
			{
				SimpleInstanceDataTarget[i] = ANIM_INSTANCE_SHADER_DATA();
			}
			*uiAnimNumPtr = 1.0f;
...
#ifdef _DEBUG
		uiGetsize += sizeof(Vector3W) * 3;
		if (HaveSkinInstance())
		{
			uiGetsize += sizeof(Vector3W) * VSResourceManager::GetSimpleInstanceAnimFloat4Num();
		}
...
uint32 VSInstanceGeometry::GetInstanceNum()
{
...
void VSInstanceGeometry::UpdateGeometryBeforeDrawCall()
{
	...
	uint32 , m_uiInstanceSize, m_uiInstanceSize,
	{

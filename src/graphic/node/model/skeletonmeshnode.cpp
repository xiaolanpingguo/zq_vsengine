...
void VSSkeletonMeshNode::CreateAnimInstanceData()
{
	...
	for (uint32 i = 0; i < m_pAnimSet->GetAnimNum(); i++)
	{
		...
		uint32 uiFrameNum = 0;
		for (float f = 0.0f; f <= pAnim->GetAnimLength() + ONE_FRAME_TIME; f += ONE_FRAME_TIME, uiFrameNum++)
		{
	...
	HalfVector3W * pTexData = m_HalfAnimData.GetBuffer();
#else
	m_AnimData.SetBufferNum(m_uiAnimInstanceTextureWidth * m_uiAnimInstanceTextureHeight);
	Vector3W * pTexData = m_AnimData.GetBuffer();	
#endif
	VSArray<Matrix3X3W> SaveBoneMatrix;
	SaveBoneMatrix.SetBufferNum(m_pSkeleton->GetBoneNum());
	for (uint32 i = 0; i < m_pSkeleton->GetBoneNum(); i++)
	{
	...
	for (uint32 i = 0; i < m_pAnimSet->GetAnimNum(); i++)
	{
		...
		for (float f = 0.0f; f <= pAnim->GetAnimLength() + ONE_FRAME_TIME; f += ONE_FRAME_TIME, uiFrameNum++)
		{
			uint32 CurFrame = LastFrame + uiFrameNum;
			uint32 Col = CurFrame % m_uiAnimInstanceTextureWidth;
			m_pAnimSequence->ClearFlag();
			m_pAnimSequence->Update(f);
			m_pAnimSequence->UpdateBone();
			m_pSkeleton->UpdateAll(0.0f);
#if USE_HALF_ANIM_DATA > 0			
			HalfVector3W * pCurTexData = NULL;
#else
			Vector3W * pCurTexData = NULL;
#endif
			pCurTexData = pTexData + Col;
			for (uint32 j = 0; j < m_pSkeleton->GetBoneNum(); j++)
			{
...
{
	uint32 uiTextureW = m_uiAnimInstanceTextureWidth;
	uint32 uiTextureH = m_uiAnimInstanceTextureHeight;
	VSArray<Vector3W> AnimDataTemp;
	VSArray<HalfVector3W> HalfAnimDataTemp;
	...
	if (!VSRenderer::ms_pRenderer->IsSupportFeature(VSRenderer::SF_NoPow2Texture))
	{
		if (!IsTwoPower(m_uiAnimInstanceTextureWidth) || !IsTwoPower(m_uiAnimInstanceTextureHeight))
		{
			...
			if (m_AnimData.GetNum())
			{
				...
				Vector3W * pDestBuffer = AnimDataTemp.GetBuffer();
				Vector3W * pSourceBuffer = m_AnimData.GetBuffer();
				for (uint32 i = 0; i < m_uiAnimInstanceTextureHeight; i++)
				{
					VSMemcpy(pDestBuffer, pSourceBuffer, uiTextureW * sizeof(Vector3W));
			...
			else if(m_HalfAnimData.GetNum())
			{
				...
				HalfVector3W * pDestBuffer = HalfAnimDataTemp.GetBuffer();
				HalfVector3W * pSourceBuffer = m_HalfAnimData.GetBuffer();
				for (uint32 i = 0; i < m_uiAnimInstanceTextureHeight; i++)
				{
					VSMemcpy(pDestBuffer, pSourceBuffer, uiTextureW * sizeof(HalfVector3W));
					pDestBuffer += uiTextureW;
					pSourceBuffer += m_uiAnimInstanceTextureWidth;
				}
...
{
	ms_Default = MEM_NEW VSSkeletonMeshNode();
...
{
	for (uint32 i = 0; i < m_pSocketArray.GetNum(); i++)
	{
...
VSSocketNode * VSSkeletonMeshNode::CreateSocket(const VSUsedName & BoneName, const VSUsedName &SocketName)
{
	...
	VSSocketNode * pSocketNode = MEM_NEW VSSocketNode();
...
#define SAMPLE_M_SCEOND 200.0f		
		Vector3 MaxPos = m_pSkeleton->m_OriginLocalBV.GetMaxPoint();
		Vector3 MinPos = m_pSkeleton->m_OriginLocalBV.GetMinPoint();
		...
		for (uint32 i = 0; i < m_pAnimSet->GetAnimNum(); i++)
		{
			...
			VSAnim * pAnim = pAnimR->GetResource();
			float AnimLength = pAnim->GetAnimLength();
			VSUsedName AnimName = m_pAnimSet->GetAnimName(i);
			PlayAnim(AnimName.GetString(), 1.0f, VSController::RT_CLAMP);

			for (float f = 0.0f; f < AnimLength + SAMPLE_M_SCEOND; f += SAMPLE_M_SCEOND)
			{
				m_pAnimSequence->ClearFlag();
				m_pAnimSequence->Update(f);
				m_pAnimSequence->UpdateBone();
...
void VSSkeletonMeshNode::UpdateWorldBound(double dAppTime)
{
	...
	if (m_pSkeleton)
	{
		...
		{
			Vector3 MaxPos = m_pSkeleton->m_WorldBV.GetMaxPoint();
			Vector3 MinPos = m_pSkeleton->m_WorldBV.GetMinPoint();

			float fA[3];
			m_WorldBV.GetfA(fA);

			MaxPos = MaxPos + Vector3(fA[0], fA[1], fA[2]);
			MinPos = MinPos - Vector3(fA[0], fA[1], fA[2]);
...
bool VSSkeletonMeshNode::PlayAnim(const VSString & AnimName,float fRatio,uint32 uiRepeatType)
{
	if (m_pAnimSequence == NULL)
	{
		m_pAnimSequence = MEM_NEW VSAnimSequenceFunc();
	...
	if (m_AnimInstanceTexture && m_bDrawInstance)
	{
		...
		{			
			...
			if (TempAnimInstanceData.GetNum() > MAX_SIMPLE_INSTANCE_ANIM)
			{
				...
				for (uint32 i = 0; i < TempAnimInstanceData.GetNum(); i++)
				{

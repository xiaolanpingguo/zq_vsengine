						   ...
						   uint32 uiVertexNum)
{
	...
	{
		pMeshData = MEM_NEW VSPointSet();
	}
	else if ( uiMeshDataType == VSMeshData::MDT_LINE)
	{
		pMeshData = MEM_NEW VSLineSet();
	}
	...
	pVertexBuffer = MEM_NEW VSVertexBuffer(FormatArray,uiVertexNum);
	...
	m_pVertexUseBuffer = MEM_NEW VSUseBuffer(false);
	...
	{
		m_pVertexUseBufferRender = MEM_NEW VSUseBuffer(false);
...
bool VSDVGeometry::Add(const void * pVeretexData,uint32 uiVertexSize)
{
...
void * VSDVGeometry::NewGetV(uint32 uiVertexSize)
{
	if (!uiVertexSize)
	{
		return 0;
...
uint32 VSDVGeometry::UpdateGeometry()
{
	...
	uint32 uiActiveNum = 0;
	uint32 uiVElementEndIndex = m_uiCurVUseBufferElementIndex;
	uint32 uiVSize = 0;
	for (uint32 i = m_uiCurVUseBufferElementIndex ; i < m_pVertexUseBufferRender->GetElementNum() ; i++)
	{
...
uint32 VSDVGeometry::GetMaxRenderVertexNum()const
{
												 ...
												 uint32 uiIndexNum)
{
	...
	VSIndexBuffer * pIndexBuffer = MEM_NEW VSIndexBuffer(uiIndexNum);
	...
	m_pIndexUseBuffer = MEM_NEW VSUseBuffer(false);
	...
	if (VSResourceManager::ms_bRenderThread)
	{
		...
		m_pIndexUseBufferRender = MEM_NEW VSUseBuffer(false);
...
void * VSDVDIGeometry::NewGetV(uint32 uiVertexSize)
{
	if (!uiVertexSize)
	{
		return 0;
...
void * VSDVDIGeometry::NewGetI(uint32 uiIndexSize)
{
	if (!uiIndexSize)
	{
		return 0;
...
bool VSDVDIGeometry::Add(const void * pVeretexData,uint32 uiVertexSize,
								  const void * pIndexData,uint32 uiIndexSize)
{
	...
	uint32 uiLastVertexNum = 0;
	uint32 uiVElementNum = m_pVertexUseBuffer->GetElementNum();
	if (uiVElementNum)
	{
		VSVertexBuffer * pVertexBuffer = m_pMeshData->GetVertexBuffer();
		if (pVertexBuffer)
	...
	uint32 uiIElementNum = m_pIndexUseBuffer->GetElementNum();
	pBuffer += m_pIndexUseBuffer->GetElementInfo(uiIElementNum - 1)->m_uiStart;

	MATH_USHORT_INDEX * pIndexDataBuffer = (MATH_USHORT_INDEX *)pBuffer;
	for ( ; (uint64)pIndexDataBuffer < (uint64)m_pIndexUseBuffer->GetElementInfo(uiIElementNum - 1)->m_uiEnd ; pIndexDataBuffer++)
	{
		*pIndexDataBuffer += uiLastVertexNum;
	}
	if ((uint64)pIndexDataBuffer != (uint64)m_pIndexUseBuffer->GetElementInfo(uiIElementNum - 1)->m_uiEnd)
	{
		return false;
	}
...
uint32 VSDVDIGeometry::UpdateGeometry()
{
	...
	uint32 uiIElementEndIndex = m_uiCurIUseBufferElementIndex;
	uint32 uiISize = 0;
	for (uint32 i = m_uiCurIUseBufferElementIndex ; i < m_pIndexUseBufferRender->GetElementNum() ; i++)
	{
	...
	uint32 uiVCount = uiVElementEndIndex - m_uiCurVUseBufferElementIndex;
	uint32 uiICount = uiIElementEndIndex - m_uiCurIUseBufferElementIndex;
	if ( uiVCount > uiICount)
	{
		uiVSize = 0;
		uiVElementEndIndex = m_uiCurVUseBufferElementIndex + uiICount;
		for (uint32 i = m_uiCurVUseBufferElementIndex ; i <= uiVElementEndIndex ; i++)
		{
			const VSUseBuffer::VSBufferElementInfo * pVElement = m_pVertexUseBufferRender->GetElementInfo(i);
			if (pVElement)
			{
	...
	else if(uiVCount < uiICount)
	{
		...
		for (uint32 i = m_uiCurIUseBufferElementIndex ; i <= uiIElementEndIndex ; i++)
		{
			const VSUseBuffer::VSBufferElementInfo * pVElement = m_pVertexUseBufferRender->GetElementInfo(i);
			if (pVElement)
			{
	...
	pIndexBuffer->UnLock();
	uiActiveNum = m_pMeshData->GetGirdNum(uiISize / sizeof(MATH_USHORT_INDEX));
...
void VSSVDIGeometry::Create(uint32 uiMeshDataType,VSVertexBuffer * pVertexBuffer,uint32 uiIndexNum)
{
...
void * VSSVDIGeometry::NewGetI(uint32 uiIndexSize)
{
	if (!uiIndexSize)
	{
		return 0;
...
bool VSSVDIGeometry::Add(const void * pIndexData,uint32 uiIndexSize)
{
...
uint32 VSSVDIGeometry::UpdateGeometry()
{
...
bool VSDVSIGeometry::Add(const void * pVeretexData,uint32 uiVertexSize)
{
	if (!pVeretexData || !uiVertexSize)
	{
		return 0;
...
void * VSDVSIGeometry::NewGetV(uint32 uiVertexSize)
{
	if (!uiVertexSize)
	{
		return 0;
...
uint32 VSDVSIGeometry::UpdateGeometry()
{

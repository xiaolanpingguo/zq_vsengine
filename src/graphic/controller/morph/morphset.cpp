...
bool VSMorphSet::AddMorph(VSMorph * pMorph)
{
	...
	{
		//是否存在
		for (uint32 i = 0 ; i < m_pMorphArray.GetNum() ; i++)
		{
		...
		for (uint32 i = 0 ;i < pMorph->GetBufferNum() ; i++)
		{
			if (pMorph->GetVertexNum(i))
			{
				for (uint32 j = 0 ; j < m_pMorphArray.GetNum() ; j++)
				{
...
uint32 VSMorphSet::GetMorphIndex(const VSUsedName & MorphName)const
{
	for (uint32 i = 0; i < m_pMorphArray.GetNum(); i++)
	{
		if (m_pMorphArray[i])
...
VSMorph * VSMorphSet::GetMorph(uint32 i)const
{

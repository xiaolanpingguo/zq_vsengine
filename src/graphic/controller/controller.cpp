...
VSController::VSController()
{
	...
	AddTimeEvent(MEM_NEW VSStartAnimEvent(this));
	AddTimeEvent(MEM_NEW VSEndAnimEvent(this));
}
VSController::~VSController()
{
	m_pObject = NULL;
...
{
	for (uint32 i = 0; i < m_TimeEventArray.GetNum(); i++)
	{
...
double VSController::GetTime(double fTime, double fMin,double fMax,uint32 uiRepeatType)
{
...
void VSController::DeleteTimeEvent(VSTimeEvent * pTimeEvent)
{
	...
	uint32 Index = m_TimeEventArray.FindElement(pTimeEvent);

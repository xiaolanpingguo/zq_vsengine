...
VSOneParamImmediateAnimBlend::VSOneParamImmediateAnimBlend()
{
	m_uiLastAnimIndex = MATH_MAX_UINT32;
...
bool VSOneParamImmediateAnimBlend::Update(double dAppTime)
{
	...
	if (m_uiLastAnimIndex == MATH_MAX_UINT32)
	{
	...
	if (m_uiLastAnimIndex != m_uiCurAnimIndex && m_fCrossFadingTime < 0.00001f)
	{
		...
		{
			for (uint32 i = 0; i < CurLeafNodeArray.GetNum(); i++)
			{
				bool bFound = false;
				for (uint32 j = 0; j < LastLeafNodeArray.GetNum(); j++)
				{
...
// 			{
		...
		/*	}*/
			...
			m_fCrossFadingTime += (float)m_dIntervalTime;
		}
		else
		{
			m_uiLastAnimIndex = m_uiCurAnimIndex;

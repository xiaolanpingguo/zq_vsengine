...
:VSPostEffectFunction(ShowName, pPostEffectSet)
{
	...
	pInputNode = MEM_NEW VSInputNode(VSPutNode::PET_IN, _T("InputColor"), this);
...
VSPostEffectSceneRender * VSPEMaterial::CreateSceneRender()
{
	...
	{
		m_pPostEffectRender = MEM_NEW VSPEMaterialSceneRender();
	}
...
{
	m_pMaterialInstance = MEM_NEW VSMaterialInstance(pMaterial);
}

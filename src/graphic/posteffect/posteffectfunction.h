...
#include "graphic/posteffect/posteffectmarc.h"
...
class VSStream;
class GRAPHIC_API VSPostEffectFunction : public VSObject
{
	DECLARE_RTTI;
...
public:
	...
	void GetValidLinkOutputPostEffectFunction(VSArray<VSPostEffectFunction *>& Out)
	{
		for (uint32 i = 0; i < m_pOutput.GetNum(); i++)
		{
			if (m_pOutput[i])
			{
				for (uint32 j = 0; j < m_pOutput[i]->GetInputNum(); j++)
				{

#include "graphic/asyn/enginethread.h"
#include "graphic/material/shader/userconstant.h"
#include "graphic/render/bind.h"
using namespace zq;
VSConstantBuffer::VSConstantBuffer()
{
	m_uiCurBufferP = 0;
	m_Buffer.SetBufferNum(Constant_BUFFER_SIZE);
}
VSConstantBuffer::~VSConstantBuffer()
{

}
unsigned char * VSConstantBuffer::Assign(uint32 uiSize)
{
	VSCriticalSection::Locker Temp(m_Lock);
	unsigned char * pTemp = NULL;	
	m_uiCurBufferP += uiSize;
	if (m_uiCurBufferP > Constant_BUFFER_SIZE)
	{
		VSMAC_ASSERT(0);
		return NULL;
	}

	pTemp = &m_Buffer[m_uiCurBufferP - uiSize];
	return pTemp;
}
unsigned char * VSConstantBuffer::Assign(uint32 VTType,uint32 uiRegisterNum)
{
	uint32 uiSize = 0;
	if(VTType == VSUserConstant::VT_FLOAT)
	{
		uiSize = sizeof(float) * 4 * uiRegisterNum;
	}
	else if (VTType == VSUserConstant::VT_INT)
	{
		uiSize = sizeof(int) * 4 * uiRegisterNum;
	}
	else if (VTType == VSUserConstant::VT_BOOL)
	{
		uiSize = sizeof(bool) * 4 * uiRegisterNum;
	}
	VSMAC_ASSERT(uiSize);
	return Assign(uiSize);
}
void VSConstantBuffer::Clear()
{
	m_uiCurBufferP = NULL;
}
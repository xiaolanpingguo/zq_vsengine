...
void VSFileMonitor::Run()
{
	...
	m_pReadBuffer = MEM_NEW unsigned char[BufferSize];
	...
	while (S_OK == hr)
	{
		...
		memset(m_pReadBuffer, 0,BufferSize);
		...
		if (S_OK == hr)
		{
			Synchronize::WaitAll(events,2,false);
...
const char* VSFileMonitor::GetThreadName()
{
...
VSResourceMonitor::~VSResourceMonitor()
{
	for (uint32  i = 0; i < m_handlers.GetNum(); ++i)
	{
		SAFE_DELETE(m_handlers[i]);
	}
	m_handlers.Clear();
}
...
{
	m_fileMonitor = MEM_NEW VSFileMonitor(this);
...
void VSResourceMonitor::Update(double tick)
{
	...
	for (uint32 i = 0 ; i < files.GetNum(); i++)
	{
		...
		for (uint32 j = 0; j < m_handlers.GetNum(); ++j)
		{

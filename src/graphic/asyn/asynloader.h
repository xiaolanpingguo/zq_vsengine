...
namespace zq{
	...
	class GRAPHIC_API VSAsynJob : public MemObject
	{
	public:
		VSAsynJob();
		virtual ~VSAsynJob() = 0;
		...
		uint32 m_uiControllState;
		uint32 m_uiJobState;
		virtual void MainThreadProcess() = 0;
		virtual void AsynThreadProcess() = 0;
		virtual uint32 GetJobType()const = 0;
		virtual void Failed() = 0;
	};
	class GRAPHIC_API VSFileJob :public VSAsynJob
	{
	public:
		VSFileJob();
		virtual ~VSFileJob() = 0;
		...
		virtual uint32 GetJobType()const
		{
			return JT_FILE;
		}
	...
	class GRAPHIC_API VSResourceJob : public VSFileJob
	{
	public:
		enum 
		{
	...
	class GRAPHIC_API VSCacheResourceJobBase : public VSResourceJob
	{
	public:
	...
	class GRAPHIC_API VSCacheResourceJob : public VSCacheResourceJobBase<CacheType>
	{
	public:
		...
		virtual uint32 GetResourceType()const
		{
	...
	template<typename T, typename CacheType>
	class GRAPHIC_API VSStreamResourceJob : public VSCacheResourceJobBase<CacheType>
	{
	public:
...
template<typename T>
class GRAPHIC_API VSTResourceJob : public VSResourceJob
{	
public:
...
class GRAPHIC_API VSAsynJobThread : public Thread
{
public:
	VSAsynJobThread();
...
protected:
	...
	{
		return "VSAsynJobThread";
	}
	VSSafeQueue<VSAsynJob*> m_pResourceQueue;
	virtual void Run();	
};
	
class GRAPHIC_API VSASYNLoadManager
{
public:
	enum MyEnum
	{
	...
	{
		VSFileJob* pResourceProxyJob = MEM_NEW VSTResourceJob<T>(ResourceRPTR);
	...
	{
		VSStreamResourceJob<T, typename T::CacheType>* pResourceCacheJob = MEM_NEW VSStreamResourceJob<T, typename T::CacheType>(pResourceR);
		pResourceCacheJob->m_FileName = pResourceR->GetResource()->GetCacheFilePath();
		AddJob(pResourceCacheJob);
	}
protected:
	void PendingResource(float fTime);
	template<typename T>
	VSCacheResourceJob<T,typename T::CacheType> * AddCacheResourceJob(T * pResource)
	{
		VSCacheResourceJob<T, typename T::CacheType>* pResourceCacheJob = MEM_NEW VSCacheResourceJob<T, typename T::CacheType>(pResource);

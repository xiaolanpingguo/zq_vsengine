...
VS2DTextureArray::VS2DTextureArray(uint32 uiWidth, uint32 uiHeight, uint32 uiFormatType,
	uint32 uiMipLevel, uint32 uiArraySize, bool bIsStatic, bool bSRGB)
...
VS2DTextureArray::VS2DTextureArray(VSArray<VS2DTexture *> &pTextureArray)
{
	...
	uint32 uiFormat = 0;
	uint32 uiMipLevel = 0;
	uint32 uiOutputType = VSOutputResource::OT_NONE;
	bool bStatic = true;
	bool bSRGB = false;
	for (uint32 i = 0; i < pTextureArray.GetNum(); i++)
	{
		if (pTextureArray[i])
		{
			if (!bFind)
	...
	if (uiOutputType == VSOutputResource::OT_NONE)
	{
		...
		CreateRAMData();
		for (uint32 i = 0; i < m_uiArraySize; i++)
		{
			if (pTextureArray[i])
			{
				for (uint32 j = 0; j < m_uiMipLevel; j++)
				{
			...
			else
			{
				for (uint32 j = 0; j < m_uiMipLevel; j++)
				{
					memset(GetBuffer(j, i), 0, GetByteSize(j));
				}
			}
		}

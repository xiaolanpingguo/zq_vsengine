...
void VSCaptureTexAllState::SetMipLevel(uint32 uiMipLevel)
{
...
void VSCaptureTexAllState::CreateStaticTexture()
{
	...
	{
		VSTexture * pTexture =  pCaptureViewFamily->GetTexture();
		uint32 uiTextureType = pTexture->GetTexType();
		if (uiTextureType == VSTexture::TT_2D)
		{
			m_pStaticTexture = MEM_NEW VS2DTexture(pTexture->GetWidth(0),pTexture->GetHeight(0),pTexture->GetFormatType(),m_uiMipLevel,1);
		}
		else if (uiTextureType == VSTexture::TT_CUBE)
		{
			m_pStaticTexture = MEM_NEW VSCubeTexture(pTexture->GetWidth(0),pTexture->GetFormatType(),m_uiMipLevel,1);
		}
	...
	if (!m_bDynamic)
	{
		...
		if (pSourceTexture->GetTexType() == VSTexture::TT_2D)
		{
				...
				VSTexture *,pSourceTexture,pSourceTexture,VSTexture *,pDestTexture,pDestTexture,
			{
				...
				for (uint32 i = 1 ; i < pDestTexture->GetMipLevel() ; i++)
				{

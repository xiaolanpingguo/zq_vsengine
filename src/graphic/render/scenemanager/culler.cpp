...
bool VSCuller::PushPlane(const Plane & Plane)
{
...
{
	Plane plane[VSCamera::CP_MAX];
	Camera.GetPlane(plane);
	for(uint32 i =0 ; i < VSCamera::CP_MAX; i++)
	{
		if(!PushPlane(plane[i]))
			return 0;
	}

	VSArray<Plane> & CustomCullPlane = Camera.m_CustomCullPlane;
	for (uint32 i = 0; i < CustomCullPlane.GetNum(); i++)
	{
...
bool VSCuller::PopPlane(Plane &plane)
{
...
bool VSCuller::InsertObject(VSRenderContext &VisibleContext,uint32 uiVisibleSetType,uint32 uiRenderGroup)
{
...
void VSCuller::GetAndSortLight()
{
	...
	for (uint32 uiRenderGroup = 0; uiRenderGroup < RG_MAX; uiRenderGroup++)
	{
		for (uint32 t = 0; t <= VST_MAX; t++)
		{
			for (uint32 j = 0; j < GetVisibleNum(t, uiRenderGroup); j++)
			{
				VSRenderContext& VisibleContext = GetVisibleSpatial(j, t, uiRenderGroup);

				VSGeometry * pGeometry = VisibleContext.m_pGeometry;

				for (uint32 l = 0; l < GetLightNum(); l++)
				{
...
uint32 VSCuller::IsVisible(const Sphere &S,bool bClearState)
{
	VSMAC_ASSERT(m_uiPlaneNum);
	int32 iP = m_uiPlaneNum - 1;
	uint32 uiMask = 1 << iP;
	uint32 uiPlaneInNum = 0;
	for (uint32 i = 0; i < m_uiPlaneNum; i++, iP--, uiMask >>= 1)
	{
		if (m_uiPlaneState & uiMask)
		{
			int32 iSide = S.RelationWith(m_Plane[iP]);
...
uint32 VSCuller::IsVisible(const Vector3 &Point,bool bClearState)
{
	VSMAC_ASSERT(m_uiPlaneNum);
	int32 iP = m_uiPlaneNum - 1;
	uint32 uiMask = 1 << iP;
	...
	for (uint32 i = 0; i < m_uiPlaneNum; i++, iP--, uiMask >>= 1)
	{
		if (m_uiPlaneState & uiMask)
		{
			int32 iSide = Point.RelationWith(m_Plane[iP]);
...
uint32 VSCuller::IsVisible(const AABB &BV,bool bClearState)
{
	VSMAC_ASSERT(m_uiPlaneNum);
	int32 iP = m_uiPlaneNum - 1;
	uint32 uiMask = 1 << iP;
	...
	for (uint32 i = 0; i < m_uiPlaneNum; i++, iP--, uiMask >>= 1)
	{
		if (m_uiPlaneState & uiMask)
		{
			int32 iSide = BV.RelationWith(m_Plane[iP]);
...
{
	for (uint32 i = 0; i < pSceneArray.GetNum(); i++)
	{
		...
		for (uint32 i = 0; i < pScene->GetAllLightNum(); i++)
		{
...
bool VSCuller::RenderPriority::operator()(VSRenderContext & p1,VSRenderContext & p2)
{
	...
	int32 uiMaterialCompareFlag = ComparePointer(p1.m_pMaterialInstance->GetMaterial(), p2.m_pMaterialInstance->GetMaterial());

	int32 uiMeshDataCompareFlag = ComparePointer(p1.m_pGeometry->GetMeshData(), p2.m_pGeometry->GetMeshData());
...
void VSCuller::CollectDynamicInstance(VSArray<VSRenderContext> & inContext)
{
	...
	uint32 uiEndIndex = uiStartIndex + 1;
	uint32 CollectNum = 1;
	uint32 uiMaxPassID = 0;
	while (uiEndIndex < inContext.GetNum())
	{
		...
		uint32 uiTempPassID = inContext[uiEndIndex].m_uiPassId;
		...
		{		
			if (CollectNum > m_uiBeignDynamicInstanceNum)
			{
				...
				for (uint32 uiInstanceIndex = uiStartIndex; uiInstanceIndex < uiEndIndex; uiInstanceIndex++)
				{
				...
				for (uint32 uiPassID = 0; uiPassID <= uiMaxPassID; uiPassID++)
				{
...
bool VSCuller::AlphaPriority::operator()(VSRenderContext & p1,VSRenderContext & p2)
{
	...
	Vector3 vLength1 = m_pCamera->GetWorldTranslate() - p1.m_pGeometry->GetWorldTranslate(); 
	Vector3 vLength2 = m_pCamera->GetWorldTranslate() - p2.m_pGeometry->GetWorldTranslate();
...
void VSCuller::GetAllVisibleAABB(uint32 uiVisibleSetType, uint32 uiRenderGroup, VSArray<AABB> & AABBArray)
{

	for (uint32 i = 0 ; i < m_VisibleSet[uiRenderGroup][uiVisibleSetType].GetNum() ;i++)
	{
...
bool VSShadowCuller::TestObjectWithCameraIntersectOnLightDirPlane(const VSSpatial * pSpatial)
{
	...
	AABB  ObjectAABB = pSpatial->GetWorldAABB();
	AABB ReceiveAABB = m_pCamera->GetFrustumAABB();
	Vector3 ObjectToReceive = ReceiveAABB.GetCenter() - ObjectAABB.GetCenter();

	VSDirectionLight * pDirLight = (VSDirectionLight *)m_pLocalLight;
	Vector3 Dir, Up, Right;
	pDirLight->GetWorldDir(Dir, Up, Right);
	float ProjectOnDir = ObjectToReceive.Dot(Dir);
	if (ProjectOnDir < 0.0f)
	{
		return false;
	}
	
	Vector3 ProjectToReceiveCenterPlane = ObjectToReceive - (Dir * ProjectOnDir);

	float ReceiveRadius = (ReceiveAABB.GetMaxPoint() - ReceiveAABB.GetCenter()).GetLength();
	float ObjectRadius = (ObjectAABB.GetMaxPoint() - ObjectAABB.GetCenter()).GetLength();
	float TestDistance = ProjectToReceiveCenterPlane.GetSqrLength();

	float CombineRadius = ReceiveRadius + ObjectRadius;
...
bool VSVolumeShadowMapCuller::ForceNoCull(const VSSpatial * pSpatial)
{
	if (m_pLocalLight->GetLightType() == VSLight::LT_POINT)
	{
		...
		AABB AABB(pPointLight->GetWorldTranslate(), pPointLight->GetRange(), pPointLight->GetRange(), pPointLight->GetRange());
		if (AABB.RelationWith(pSpatial->GetWorldAABB()) == MATH_INTERSECT)
		{
			return true;
		}
	}
	...
	{
		AABB  ObjectAABB = pSpatial->GetWorldAABB();
		int32 iSide = ObjectAABB.RelationWith(m_Plane[VSCamera::CP_FAR]);
		if (iSide == MATH_FRONT)
		{
			return false;
		}
...
{
	for (uint32 i = 0; i < m_ElementArray.GetNum(); i++)
	{
		SAFE_DELETE(m_ElementArray[i].pQuery);
	}
...
VSOcclusionQueryType VSOcclusionQueryPool::GetOcclusionQuery()
{
	...
	{
		uint32 ID = m_ElementArray.AddElement(VSOcclusionQueryType());
		m_ElementArray[ID].ID = ID;
		m_ElementArray[ID].pQuery = MEM_NEW VSQuery(VSQuery::QT_OCCLUSION);
...
{
	uint32 uiCullIndex = m_CullerArray.FindElement(pCuller);
	...
	for (uint32 uiRenderGroup = 0; uiRenderGroup < VSCuller::RG_MAX; uiRenderGroup++)
	{
		for (uint32 t = 0; t < VSCuller::VST_MAX; t++)
		{
			...
			for (uint32 j = 0; j < pCuller->GetVisibleNum(t, uiRenderGroup); j++)
			{
				...
				uint32 uiIndex = m_NoVisibleGeometry[uiRenderGroup][uiCullIndex].FindElement(pGeometry);
...
{
	for (uint32 i = 0 ; i < m_PendingRegister.GetNum() ; i++)
	{
		OnRegisterCuller(m_PendingRegister[i]);
	}
	m_PendingRegister.Clear();
	for (uint32 i = 0; i < m_PendingDelete.GetNum(); i++)
	{
...
bool VSOcclusionQueryType::GetQueryData(void * pData, uint32 uiDataSize)
{
...
void VSCullerManager::GetQueryData()
{
	...
	for (uint32 uiCullIndex = 0; uiCullIndex < m_CullerArray.GetNum(); uiCullIndex++)
	{
		...
		unsigned int, uiCullIndex, uiCullIndex,
			{
				for (uint32 uiRenderGroup = 0; uiRenderGroup < VSCuller::RG_MAX; uiRenderGroup++)
				{
					for (uint32 i = 0; i < (*(GOQBufferPointerGroup.GOQBufferPointer[uiRenderGroup]))[uiCullIndex].GetNum(); i++)
					{
						...
						{
							OQD.uiPixedNum = MATH_MAX_UINT32;
						}
						else
						{
							OQD.uiPixedNum = Data;
...
{		
	...
	for (uint32 uiCullIndex = 0; uiCullIndex < m_CullerArray.GetNum(); uiCullIndex++)
	{
		for (uint32 uiRenderGroup = 0; uiRenderGroup < VSCuller::RG_MAX; uiRenderGroup++)
		{		
			for (uint32 i = 0; i < (*(m_GOQBufferPointer[uiRenderGroup][0]))[uiCullIndex].GetNum(); i++)
			{
...
VSArray<VSGeometryOcclusionQueryData> & VSCullerManager::GetGroupGeometryOcclusionQueryData(VSCuller * pCuller, uint32 uiGroup)
{
	uint32 uiCullIndex = m_CullerArray.FindElement(pCuller);
	if (uiCullIndex == m_CullerArray.GetNum())
	{

...
void VSScene::Update(double dAppTime)
{
	if (m_bIsBuild == true)
	{
		...
		for (uint32 i = 0; i < m_pDynamic.GetNum(); i++)
		{
	...
	{
		for (uint32 i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			ENQUEUE_LAMBDA_UPDATE_COMMAND(VSUpdateDynamicNode)
			([this, dAppTime, i]()
			{
	...
	VSMAC_ASSERT(pObject);
	{
		...
		for (uint32 i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			if (pObject == m_ObjectNodes[i])
			{
				...
				if (m_bIsBuild)
				{
					for (uint32 j = 0; j < m_pDynamic.GetNum(); j++)
					{
...
void VSScene::GetStreamResourceWithPos(const Vector3& CameraPos, const Vector3W& ProjectInfo)
{
...
{	
	...
	m_pStaticRoot = MEM_NEW VSQuadNode();
	...
	CollectUpdateInfo();
	for (uint32 i = 0; i < m_pAllLight.GetNum(); i++)
	{
...
uint32 VSQuadNode::AddChild(VSSpatial * pChild)
{
	uint32 id = VSNode::AddChild(pChild);
...
uint32 VSQuadNode::DeleteChild(VSSpatial *pChild)
{
	uint32 id = VSNode::DeleteChild(pChild);
...
bool VSQuadNode::DeleteChild(uint32 i)
{
...
VSNodeComponent *VSQuadNode::GetNeedDrawNode(uint32 uiIndex)const
{
...
uint32 VSQuadNode::GetNeedDrawNodeNum()const
{
...
void VSQuadNode::ComputeNodeVisibleSet(VSCuller & Culler, bool bNoCull, double dAppTime)
{
	...
	{
		for (uint32 i = 0; i < m_pNeedDrawNode.GetNum(); i++)
		{
...
void VSQuadNode::AddNeedDrawNode(VSNodeComponent *pNeedDrawNode)
{
	...
	uint32 i = m_pNeedDrawNode.FindElement(pNeedDrawNode);
...
bool VSQuadNode::RecursiveBuild(const VSArray<VSSpatial *> &pObjectArray)
{
	...
	{
		for (uint32 i = 0 ; i < pObjectArray.GetNum() ; i++)
		{
	...
	for (uint32 i = 0 ; i < pObjectArray.GetNum() ; i++)
	{
		if (pObjectArray[i])
		{
			const AABB & WorldAABB = pObjectArray[i]->GetWorldAABB();
	...
	for (uint32 i = 0 ; i < pObjectArray.GetNum() ; i++)
	{
		...
		{
			if (ChildAABB[j].RelationWith(pObjectArray[i]->GetWorldTranslate()) != MATH_OUT)
			{
	...
	for (uint32 i = 0 ; i < 4 ; i++)
	{
		...
		{
			VSQuadNode * pQuadNode = MEM_NEW VSQuadNode();
			AddChild(pQuadNode);

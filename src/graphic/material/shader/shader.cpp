...
END_ADD_PROPERTY
VSShader::VSShader(const char * pBuffer,const VSString & MainFunName,bool IsFromFile)
:VSBind()
{
...
VSShader::~VSShader()
{
	SAFE_DELETEA(m_pCacheBuffer);
	m_uiCacheBufferSize = 0;
	ReleaseResource();
}
void VSShader::SetShaderString(const char * pBuffer,const VSString & MainFunName,bool IsFromFile)
{
	SAFE_DELETEA(m_pCacheBuffer);
	m_uiCacheBufferSize = 0;
	ReleaseResource();
...
bool VSShader::SetParam(const VSUsedName &Name,VSTexAllState * pTexture,uint32 uiIndex)
{
	if (!pTexture || !m_bCreatePara)
	{
		return false;
	}
	for (uint32 i = 0 ; i < m_pUserSampler.GetNum() ;i++)
	{
...
bool VSShader::SetParam(const VSUsedName &Name, VSBufferResource * pBufferResource, uint32 uiIndex)
{
	if (!pBufferResource || !m_bCreatePara)
	{
		return false;
	}
	for (uint32 i = 0; i < m_pUserBuffer.GetNum(); i++)
	{
...
bool VSShader::SetParam(const VSUsedName &Name,void * pData)
{
	...
	for (uint32 i = 0 ; i < m_pUserConstant.GetNum() ;i++)
	{
...
bool VSShader::SetCacheBuffer(void * pBuffer,uint32 uiSize)
{
	if (!pBuffer || !uiSize)
	{
		return false;
	...
	m_pCacheBuffer = MEM_NEW UCHAR[uiSize];
	VSMemcpy(m_pCacheBuffer,pBuffer,uiSize);
	return 1;
}
void VSShader::AddShareSampler(uint32 ShareSamplerRegister, uint32 ShareSamplerType)
{
	...
	for (uint32 i = 0 ; i < m_uiShareSamplerRegister.GetNum() ; i++)
	{

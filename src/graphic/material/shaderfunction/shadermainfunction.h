...
{	class VSLight;
	DECLARE_Ptr(VSLight);
	class GRAPHIC_API VSShaderMainFunction : public VSShaderFunction
	{
		//RTTI
		DECLARE_RTTI;
	...
	public:
		...
		virtual inline uint32 GetSMType()const = 0;
		...
		inline void AddClipPlane(const Plane & Plane)
		{
		...
		inline void SetAlphaTestValue(float AlphaTestValue)
		{
	...
	protected:
		...
		void GetValueUseDeclareString(VSString &OutString, uint32 uiValueUseString, MaterialShaderPara &MSPara);
		VSRenderState	m_RenderState;

		float m_fAlphaTestValue;

		void GetValueUseString(VSString &OutString, uint32 uiValueUseString, MaterialShaderPara &MSPara);

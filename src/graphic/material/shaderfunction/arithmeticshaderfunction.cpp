...
:VSShaderFunction(ShowName,pMaterial)
{
	...
	VSOutputNode * pOutputNode = NULL;
	pOutputNode = MEM_NEW VSOutputNode(VSPutNode::VT_1,OutputName,this);
	VSMAC_ASSERT(pOutputNode);
	m_pOutput.AddElement(pOutputNode);
	VSShaderStringFactory::ms_ShaderValueIndex++;
}
...
IMPLEMENT_INITIAL_END
VSMakeValue::VSMakeValue(const VSUsedName & ShowName, VSMaterial * pMaterial, uint32 uiInputNum)
:VSShaderFunction(ShowName, pMaterial)
{

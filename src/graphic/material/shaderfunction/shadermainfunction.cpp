...
{
	uint32 uiOutputStringType = MSPara.uiPassType;
	if (m_bIsVisited == 1)
		return 1;
	else
	{
...
void VSShaderMainFunction::GetValueUseDeclareString(VSString &OutString, uint32 uiValueUseString, MaterialShaderPara &MSPara)
{
...
void VSShaderMainFunction::GetLightShadow(MaterialShaderPara& MSPara,VSArray<VSString> ShadowStringArray[VSLight::LT_MAX])const
{
	...
	uint32 uiShadowNum = 0;
	uint32 uiLightIndex[VSLight::LT_MAX] = { 0 };	
	for (uint32 i = 0 ; i< pLightArray.GetNum() ; i++)
	{
...
void VSShaderMainFunction::GetValueUseString(VSString &OutString, uint32 uiValueUseString, MaterialShaderPara &MSPara)
{
	if ((uiValueUseString & VUS_WORLD_NORMAL) == VUS_WORLD_NORMAL)
	{
...
{
	VSString Temp;
	uint32 uiTessellationValueType = GetTessellationValueNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiWorldDisplacementValueType = GetWorldDisplacementNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiWorldOffsetValueType = GetWorldOffsetNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiNormalValueType = GetNormalNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiAplhaValueType = GetAlphaNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiEmissiveType = GetEmissiveNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiDiffuseType = GetDiffuseNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiReflectMipValueType = GetReflectMipNode()->GetValueType();
...
{
	VSString Temp;
	uint32 uiReflectPowValueType = GetReflectPowNode()->GetValueType();

...
:VSShaderMainFunction(ShowName,pMaterial)
{
	...
	InputName = _T("Alpha");
	pInputNode = NULL;
	pInputNode = MEM_NEW VSInputNode(VSPutNode::VT_1,InputName,this);
	VSMAC_ASSERT(pInputNode);
	m_pInput.AddElement(pInputNode);
	...
	pInputNode = NULL;
	pInputNode = MEM_NEW VSInputNode(VSPutNode::VT_3, InputName, this);
	VSMAC_ASSERT(pInputNode);
	m_pInput.AddElement(pInputNode);
...
bool VSCustomShaderFunction::GetFunctionString(VSString &OutString, MaterialShaderPara &MSPara)const
{
	...
	else
	{
		...
		int32 iLightNum[VSLight::LT_MAX] = { 0 };	
		for (uint32 i = 0 ; i < MSPara.LightArray.GetNum() ; i++)
		{
...
void VSCustomShaderFunction::CreatLightFunctionString(MaterialShaderPara &MSPara)
{
	...
	if (m_pInput[IN_CUSTOM]->GetOutputLink())
	{
		...
		for (uint32 i = 0 ; i < NoLightFunctionParentNodeArray.GetNum() ; i++)
		{
	...
	uint32 uiCustomValueType = m_pInput[IN_CUSTOM]->GetValueType();
	VSString Temp;
	...
	for (uint32 i = 0 ; i < NoLightFunctionParentNodeArray.GetNum() ; i++)
	{
		...
		uint32 VTType = NoLightFunctionParentNodeArray[i]->GetOutputNode(0)->GetValueType();

		VSString TypeString;
		if(VTType == VSPutNode::VT_1)
		{
			TypeString +=VSRenderer::ms_pRenderer->Float() + _T(" ");/*_T("float ");*/
		}

...
#define ms_LightShadowMatrix ms_ViewProjectMatrix
	class GRAPHIC_API VSShaderStringFactory
	{
	public:	
		...
		static uint32 ms_uiShareSamplerRegisterID[VSEngineFlag::ms_uiShaderSamplerNum];
		static uint32 GetMaxShareSamplerRegisterID();

		static void AddShaderSamplerRegisterID(VSShader * pShader, uint32 uiTexRegisterID, uint32 ShareSamplerType);
		static uint32 ms_uiCreateVShaderNum;
		static uint32 ms_uiCreatePShaderNum;
		static uint32 ms_uiCreateGShaderNum;
		static uint32 ms_uiCreateHShaderNum;
		static uint32 ms_uiCreateDShaderNum;

		static uint32 ms_ShaderValueIndex;

		
		//Create Shader


		static bool CreateVShaderString(VSVShader * pVShader,MaterialShaderPara &MSPara,uint32 uiShaderID,VSString & VShaderString
#if _DEBUG 
			,VSString &OutShaderFileName);
#else
			);
#endif
		static bool CreatePShaderString(VSPShader * pPShader,MaterialShaderPara &MSPara,uint32 uiShaderID,VSString & PShaderString
#if _DEBUG 
			,VSString &OutShaderFileName);
#else
			);
		...
		static bool CreateGShaderString(VSGShader * pGShader, MaterialShaderPara &MSPara,uint32 uiShaderID, VSString & GShaderString
#if _DEBUG 
			, VSString &OutShaderFileName);
#else
			);
#endif
		static bool CreateHShaderString(VSHShader * pHShader, MaterialShaderPara &MSPara, uint32 uiShaderID, VSString & HShaderString
#if _DEBUG 
			, VSString &OutShaderFileName);
#else
			);
#endif
		static bool CreateDShaderString(VSDShader * pDShader, MaterialShaderPara &MSPara, uint32 uiShaderID, VSString & DShaderString
#if _DEBUG 
			, VSString &OutShaderFileName);
#else
			);
...
#if _DEBUG 
		static void OutPutShaderLog(MaterialShaderPara &MSPara, uint32 uiShaderID,

...
#include "graphic/material/shaderfunction/customshaderfunction.h"
#include "graphic/material/shaderfunction/lightshaderfunction.h"
...
#include "graphic/material/shaderfunction/arithmeticshaderfunction.h"
#include "graphic/render/colorbuffer.h"
using namespace zq;
...
{
	ms_pTestStruct = MEM_NEW VSCustomCSMaterial();
	ms_pTestByteBuffer = MEM_NEW VSCustomCSMaterial();
	ms_pTestTexture = MEM_NEW VSCustomCSMaterial();
	LoadDefault();
	return 1;
}
...
VSCustomCSMaterial::VSCustomCSMaterial(const VSUsedName & ResourceName,
	const char * pVShaderFileName, const char * pVMainFunName)
{
	m_pCShaderSet = NULL;
	m_pCShader = NULL;
...
void VSCustomCSMaterial::PreLoad(const VSUsedName & ResourceName,
	const char * pCShaderFileName, const char * pCMainFunName)
{
...
bool VSCustomCSMaterial::GetCShader(VSShaderKey & CShaderKey)
{
	...
	if (m_pCShader == NULL)
	{
		if (m_pCShaderSet)
		{
			uint32 uiIndex = m_pCShaderSet->Find(CShaderKey);
		...
		if (pCShader == NULL)
		{
			pCShader = MEM_NEW VSCShader(m_CShaderName.GetBuffer(), m_CMainFunName.GetBuffer(), true);
...
void VSCustomCSMaterial::SetUnorderAccesses()
{
	...
	uint32 uiUAVNum = m_pUnorderAccessArray.GetNum() > UNORDERACCESSS_LEVEL ? UNORDERACCESSS_LEVEL : m_pUnorderAccessArray.GetNum();

	for (uint32 i = 0; i < uiUAVNum; i++)
	{
...
bool VSCustomCSMaterial::UseMaterial(uint32 uiThreadGroupCountX, uint32 uiThreadGroupCountY, uint32 uiThreadGroupCountZ)
{
...
{
	ms_pPostGray = MEM_NEW VSCustomMaterial();
	ms_pGammaCorrect = MEM_NEW VSCustomMaterial();
	ms_pPostScreenQuad = MEM_NEW VSCustomMaterial();
	ms_pPostSaturation = MEM_NEW VSCustomMaterial();
	ms_pOldPhoto = MEM_NEW VSCustomMaterial();
	ms_pGuassBlurH = MEM_NEW VSCustomMaterial();
	ms_pGuassBlurV = MEM_NEW VSCustomMaterial();
	ms_pFilter3X3 = MEM_NEW VSCustomMaterial();
	ms_pBright = MEM_NEW VSCustomMaterial();
	ms_pTexAdd = MEM_NEW VSCustomMaterial();

	ms_pPostVolumeShadowMap = MEM_NEW VSCustomMaterial();
	ms_pProjectShadowPre = MEM_NEW VSCustomMaterial();
	ms_pProjectShadow = MEM_NEW VSCustomMaterial();
	ms_pSSR = MEM_NEW VSCustomMaterial();
	ms_pFont = MEM_NEW VSCustomMaterial();

	ms_pClearColorRange = MEM_NEW VSCustomMaterial();
	ms_pClearDepthRange = MEM_NEW VSCustomMaterial();
	ms_pClearColorDepthRange = MEM_NEW VSCustomMaterial();
	ms_pClearAllRange = MEM_NEW VSCustomMaterial();
	ms_pClearDepthStencilRange = MEM_NEW VSCustomMaterial();
	ms_pOcclusionCull = MEM_NEW VSCustomMaterial();
...
void VSCustomMaterial::AddClipPlane(const Plane & Plane)
{
...
VSCustomMaterial::VSCustomMaterial(const VSUsedName & ResourceName,
	const char * pVShaderFileName,const char * pVMainFunName,
	const char * pPShaderFileName, const char * pPMainFunName,
	const char * pGShaderFileName, const char * pGMainFunName,
	const char * pHShaderFileName, const char * pHMainFunName,
	const char * pDShaderFileName, const char * pDMainFunName)
{
		m_pVShaderSet = NULL;
		m_pPShaderSet = NULL;
		m_pGShaderSet = NULL;
...
bool VSCustomMaterial::GetVShader(VSShaderKey & VShaderKey)
{
	...
	if (m_pVShader == NULL)
	{
		if (m_pVShaderSet)
		{
			uint32 uiIndex = m_pVShaderSet->Find(VShaderKey);
		...
		{
			pVertexShader = MEM_NEW VSVShader(m_VShaderName.GetBuffer(),m_VMainFunName.GetBuffer(),true);
			if (!pVertexShader)
			{
				return 0;
			}
...
bool VSCustomMaterial::GetHShader(VSShaderKey & HShaderKey)
{
	...
	if (m_pHShader == NULL)
	{
		if (m_pHShaderSet)
		{
			uint32 uiIndex = m_pHShaderSet->Find(HShaderKey);
		...
		if (pHShader == NULL)
		{
			pHShader = MEM_NEW VSHShader(m_HShaderName.GetBuffer(), m_HMainFunName.GetBuffer(), true);
...
bool VSCustomMaterial::GetDShader(VSShaderKey & DShaderKey)
{
	...
	if (m_pDShader == NULL)
	{
		if (m_pDShaderSet)
		{
			uint32 uiIndex = m_pDShaderSet->Find(DShaderKey);
		...
		if (pDShader == NULL)
		{
			pDShader = MEM_NEW VSDShader(m_DShaderName.GetBuffer(), m_DMainFunName.GetBuffer(), true);
...
bool VSCustomMaterial::GetGShader(VSShaderKey & GShaderKey)
{
	...
	if (m_pGShader == NULL)
	{
		if (m_pGShaderSet)
		{
			uint32 uiIndex = m_pGShaderSet->Find(GShaderKey);
		...
		if (pGShader == NULL)
		{
			pGShader = MEM_NEW VSGShader(m_GShaderName.GetBuffer(), m_GMainFunName.GetBuffer(), true);
...
bool VSCustomMaterial::GetPShader(VSShaderKey & PShaderKey)
{
	...
	if (m_pPShader == NULL)
	{
		if (m_pPShaderSet)
		{
			uint32 uiIndex = m_pPShaderSet->Find(PShaderKey);
		...
		{
			pPixelShader = MEM_NEW VSPShader(m_PShaderName.GetBuffer(),m_PMainFunName.GetBuffer(),true);
...
void VSMaterial::SetPShaderValue(const VSUsedName & Name,void *fValue)
{
	...
	uint32 uiIndex = m_ShaderCustomValue.FindElement(TempValue);
	if (uiIndex != m_ShaderCustomValue.GetNum())
	{
		VSMemcpy(&m_ShaderCustomValue[uiIndex].Value[0], fValue, sizeof(float)* m_ShaderCustomValue[uiIndex].Value.GetNum());
	}
...
void VSMaterial::SetPShaderTexture(const VSUsedName & TexSamplerNodeName,VSTexAllStateR * pTex)
{
	...
	uint32 uiIndex = m_ShaderCustomTex.FindElement(TempValue);
...
{
	ms_Default = MEM_NEW VSMaterial(_T("DefaultMaterial"));
	if(!ms_Default)
		return 0;
	
	ms_DefaultOnlyColor = MEM_NEW VSMaterial(_T("DefaultOnlyColorMaterial"));

	if(!ms_DefaultOnlyColor)
		return 0;

	ms_DefaultOnlyVertexColor = MEM_NEW VSMaterial(_T("DefaultOnlyVertexColorMaterial"));
	...
	ms_DefaultOnlyColorDisableDepth = MEM_NEW VSMaterial(_T("DefaultOnlyColorDisableDepthMaterial"));

	if(!ms_DefaultOnlyColorDisableDepth)
		return 0;

	ms_DefaultOnlyVertexColorDisableDepth = MEM_NEW VSMaterial(_T("DefaultOnlyVertexColorDisableDepthMaterial"));
...
{
	{
		if(!ms_Default)
		return 0;
		VS2DTexSampler* p2DTexSamplerNode = MEM_NEW VS2DTexSampler(_T("TexSampler"),ms_Default);
		p2DTexSamplerNode->SetTexture((VSTexAllStateR *)VSTexAllState::GetDefaultResource());
		VSConstFloatValue * pSpecularColor = MEM_NEW VSConstFloatValue(_T("SpecluarColor"),ms_Default,4,false);
		pSpecularColor->SetValue(0,0.5f);
		pSpecularColor->SetValue(1,0.5f);
		pSpecularColor->SetValue(2,0.5f);
		pSpecularColor->SetValue(3,1.0f);
		VSConstFloatValue * pSpecularPow = MEM_NEW VSConstFloatValue(_T("SpecularPow"),ms_Default,1,false);
	...
	}
	{
		...
		VSConstFloatValue * pEmissiveColor = MEM_NEW VSConstFloatValue(_T("EmissiveColor"), ms_DefaultOnlyColor,4,true);
...

	{
		...
		VSVertexColor * pVertexColor = MEM_NEW VSVertexColor(_T("VertexColor"), ms_DefaultOnlyVertexColor);
...

	{
		...
		VSConstFloatValue * pEmissiveColor = MEM_NEW VSConstFloatValue(_T("EmissiveColor"), ms_DefaultOnlyColorDisableDepth,4,true);
		pEmissiveColor->SetValue(0,1.0f);
		pEmissiveColor->SetValue(1,1.0f);
		pEmissiveColor->SetValue(2,1.0f);
		pEmissiveColor->SetValue(3,1.0f);
...

	{
		...
		VSVertexColor * pVertexColor = MEM_NEW VSVertexColor(_T("VertexColor"), ms_DefaultOnlyVertexColorDisableDepth);
...
{	
	...
	m_pPass[VSPass::PT_MATERIAL] = MEM_NEW VSMaterialPass();
	m_pPass[VSPass::PT_NORMALDEPTH] = MEM_NEW VSNormalDepthPass();
	m_pPass[VSPass::PT_POINT_CUBE_SHADOW] = MEM_NEW VSCubeShadowPass();
	m_pPass[VSPass::PT_POINT_VOLUME_SHADOW] = MEM_NEW VSVolumeShadowPass();
	m_pPass[VSPass::PT_PREZ] = VSPreZPass::GetDefault();
	m_pPass[VSPass::PT_SHADOW] = MEM_NEW VSShadowPass();
	m_pPass[VSPass::PT_DUAL_PARABOLOID_SHADOW] = MEM_NEW VSDualParaboloidShadowPass();
	m_pPass[VSPass::PT_LIGHT_FUNCTION] = MEM_NEW VSLightFunPass();
	m_pPass[VSPass::PT_POSTEFFECT] = MEM_NEW VSPostEffectPass();
	m_pPass[VSPass::PT_INDIRECT] = MEM_NEW VSIndirectRenderPass();
...
VSMaterial::~VSMaterial()
{
	...
	//m_pShaderFunctionArray数量就会减少，这个FOR循环中i继续加1，而m_pShaderFunctionArray.GetNum()却减少
	//导致有些节点不能删除
	//所以用一个临时变量替代，然后把这个m_pShaderFunctionArray对应的设置为空，这样删除时找不到节点，能把正节点
	//正常释放，而且m_pShaderFunctionArray.GetNum()保持不变。
	for(uint32 i = 0 ; i < m_pShaderFunctionArray.GetNum() ; i++)
	{
	...
	for (uint32 i = 0 ; i < VSPass::PT_MAX ; i++)
	{	
		m_pPass[i] = NULL;
	}
	m_ShaderCustomValue.Clear();
...
VSMaterial::VSMaterial(const VSUsedName &ShowName, uint32 uiMUT)
{
	...
	if (uiMUT == MUT_PHONE)
	{
		m_pShaderMainFunction.AddElement(MEM_NEW VSPhoneShaderFunction(_T("PSMain"),this));
		
	}
	else if (uiMUT == MUT_OREN_NAYAR)
	{
		m_pShaderMainFunction.AddElement(MEM_NEW VSOrenNayarShaderFunction(_T("PSMain"),this));
	}
	else if (uiMUT == MUT_CUSTOM)
	{
		m_pShaderMainFunction.AddElement(MEM_NEW VSCustomShaderFunction(_T("PSMain"),this));
	}
	else if (uiMUT == MUT_LIGHT)
	{
		m_pLightShaderFunction = MEM_NEW VSLightShaderFunction(_T("PSMain"),this);
	}
	else if (uiMUT == MUT_POSTEFFECT)
	{
		m_pPostEffectShaderFunction = MEM_NEW VSPostEffectShaderFunction(_T("PSMain"), this);
	}
	m_bIsAllReadyLink = false;
	m_bIsCombine = false;
	m_pPass[VSPass::PT_MATERIAL] = MEM_NEW VSMaterialPass();
...
void VSMaterial::AddPass(uint32 uiMUT)
{
...
void VSMaterial::SetTessellationPartitioning(uint32 uiTessellationPartitioning)
{
	VSMAC_ASSERT(uiTessellationPartitioning < TP_MAX);
	m_uiTessellationPartitioning = uiTessellationPartitioning;
}
void VSMaterial::SetBlendState(VSBlendState * pBlendState,uint32 uiPassId)
{
	VSMAC_ASSERT(uiPassId < m_pShaderMainFunction.GetNum());
	m_pShaderMainFunction[uiPassId]->SetBlendState(pBlendState);
}
void VSMaterial::SetDepthStencilState(VSDepthStencilState *pDepthStencilState,uint32 uiPassId)
{
	VSMAC_ASSERT(uiPassId < m_pShaderMainFunction.GetNum());
	m_pShaderMainFunction[uiPassId]->SetDepthStencilState(pDepthStencilState);
}
void VSMaterial::SetRasterizerState(VSRasterizerState *pRasterizerState,uint32 uiPassId)
{
	VSMAC_ASSERT(uiPassId < m_pShaderMainFunction.GetNum());
	m_pShaderMainFunction[uiPassId]->SetRasterizerState(pRasterizerState);
}
void VSMaterial::AddClipPlane(const Plane & Plane,uint32 uiPassId)
{
	VSMAC_ASSERT(uiPassId < m_pShaderMainFunction.GetNum());
	m_pShaderMainFunction[uiPassId]->AddClipPlane(Plane);
}
void VSMaterial::AddScissorRect(const VSRect2 & Rect,uint32 uiPassId)
{
	VSMAC_ASSERT(uiPassId < m_pShaderMainFunction.GetNum());
...
void VSMaterial::DeleteShaderFunction(VSShaderFunction *pShaderFunction)
{
	if(pShaderFunction)
	{
		...
		for (uint32 i = 0 ; i < m_pShaderFunctionArray.GetNum() ; i++)
		{
			...
			{
				for (uint32 j = 0 ; j < pShaderFunction->GetOutputNodeNum() ;j++)
				{
...
int32 VSMaterial::GetInstanceIndex(const VSConstFloatValue * pCFValue)
{
	LinkAllResource();
	for (uint32 i = 0; i < m_ShaderCustomValue.GetNum(); i++)
	{
...
void VSMaterial::GetInstanceVertexFormat(VSArray<VSVertexFormat::VERTEXFORMAT_TYPE>& FormatArray, uint32 &uiOffset)
{
	LinkAllResource();
	uint32 TypeMap[] = { VSDataBuffer::DT_FLOAT32_1, VSDataBuffer::DT_FLOAT32_2, VSDataBuffer::DT_FLOAT32_3, VSDataBuffer::DT_FLOAT32_4 };
	for (uint32 i = 0; i < m_ShaderCustomValue.GetNum(); i++)
	{
...
void VSMaterial::CreateCustomTexture(VSShader * pShader)
{
	...
	for(uint32 i = 0 ; i < m_pShaderFunctionArray.GetNum(); i++)
	{
		...
		VSUserSampler * pSampler = MEM_NEW VSUserSampler(Temp->GetShowName(),Temp->GetTexType(),RegisterID,1);
...
void VSMaterial::CreateTextureDeclare(VSString & OutString, unsigned int& uiRegisterID, MaterialShaderPara &MSPara)
{
	...
	uint32 uiShaderSamplerRegisterID = 0;
	for (uint32 i = 0 ; i < VSEngineFlag::SS_MAX ; i++)
	{
		for (uint32 j = 0 ; j < TexSampler[i].GetNum() ; j++)
		{
	...
	for (uint32 i = 1; i < VSEngineFlag::SS_MAX; i++)
	{
...
void VSMaterial::CreateCustomValue(VSShader * pShader)
{
	for(uint32 i = 0 ; i < m_pShaderFunctionArray.GetNum(); i++)
	{
		...
		UserConstantTemp = MEM_NEW VSUserConstant(Temp->GetShowName(), Temp->GetOutputNode(0)->GetNodeName(),Temp->GetSize(), uiRegisterIndex, 1, Temp->GetType());
...
void VSMaterialInstance::SetShaderValue(const VSUsedName & Name,void *fValue,uint32 uiSize)
{
	...
	for(uint32 i = 0 ; i < m_ShaderCustomValue.GetNum() ; i++)
	{
		if(m_ShaderCustomValue[i].ConstValueName == Name)
		{
			VSMemcpy(&m_ShaderCustomValue[i].Value[0],fValue,sizeof(float) * m_ShaderCustomValue[i].Value.GetNum());
			return;
		}
	}
	//如果没有
...
void VSMaterialInstance::GetInstanceData(unsigned char* pInstanceDataBuffer,uint32 &GetSize)
{
	
	VSArrayOrder<CustomFloatValue> & MaterialShaderCustomValue = m_pMaterial->GetResource()->m_ShaderCustomValue;
	for (uint32 i = 0; i < MaterialShaderCustomValue.GetNum(); i++)
	{
		bool bFind = false;
		for (uint32 j = 0; j < m_ShaderCustomValue.GetNum(); j++)
		{
			if (MaterialShaderCustomValue[i].ConstValueName == m_ShaderCustomValue[j].ConstValueName)
			{
				uint32 uiSize = m_ShaderCustomValue[j].Value.GetNum() * sizeof(float);
		...
		if (!bFind)
		{
			uint32 uiSize = MaterialShaderCustomValue[i].Value.GetNum() * sizeof(float);
			VSMemcpy(pInstanceDataBuffer,
...
:VSMaterial(ShowName)
{
	VS2DTexSampler* p2DTexSamplerNode = MEM_NEW VS2DTexSampler(_T("DiffuseTexture"),this);
	p2DTexSamplerNode->SetTexture(pTexture);
	//p2DTexSamplerNode->SetVESRGB(VSRenderer::VE_R | VSRenderer::VE_G | VSRenderer::VE_B);

	VSConstFloatValue * pSpecularColor = MEM_NEW VSConstFloatValue(_T("SpecluarColor"),this,4,false);
	pSpecularColor->SetValue(0,1.0f);
	pSpecularColor->SetValue(1,1.0f);
	pSpecularColor->SetValue(2,1.0f);
	pSpecularColor->SetValue(3,1.0f);
	VSConstFloatValue * pSpecularPow = MEM_NEW VSConstFloatValue(_T("SpecularPow"),this,1,false);
...
:VSMaterial(ShowName)
{
	...
	VS2DTexSampler *pNormalNode = MEM_NEW VS2DTexSampler(_T("NormalTexture"),this);
...
:VSMaterial(ShowName)
{
	...
	VSConstFloatValue * pDiffuseColor = MEM_NEW VSConstFloatValue(_T("DiffuseColor"), this, 4, true);
	pDiffuseColor->SetValue(0, 1.0f);
	pDiffuseColor->SetValue(1, 1.0f);
	pDiffuseColor->SetValue(2, 1.0f);
	pDiffuseColor->SetValue(3, 1.0f);

	VSMul * pMul = MEM_NEW VSMul(_T("Mul"), this);
...
:VSMaterial(ShowName)
{
	...
	VSConstFloatValue * pEmissiveColor = MEM_NEW VSConstFloatValue(_T("EmissiveColor"),this,4,true);
	pEmissiveColor->SetValue(0,1.0f);
	pEmissiveColor->SetValue(1,1.0f);
	pEmissiveColor->SetValue(2,1.0f);
	pEmissiveColor->SetValue(3,1.0f);
...
{
	// float One = 1.0f;
	VSConstFloatValue * pOne = MEM_NEW VSConstFloatValue(_T("One"), this, 1, false);
	pOne->SetValue(0, 1.0f);

	// WorldPos
	VSWorldPos * pWorldPos = MEM_NEW VSWorldPos(_T("WorldPos"), this);

	//WorldPos_One = float4(WorldPos,1.0f);
	VSMakeValue * pWorldPos_One = MEM_NEW VSMakeValue(_T("WorldPos_One"), this, 2);
	...
	VSConstFloatValue * pReflectViewProject_1 = MEM_NEW VSConstFloatValue(_T("ReflectViewProject_1 "), this, 4, true);
	VSConstFloatValue * pReflectViewProject_2 = MEM_NEW VSConstFloatValue(_T("ReflectViewProject_2 "), this, 4, true);
	VSConstFloatValue * pReflectViewProject_3 = MEM_NEW VSConstFloatValue(_T("ReflectViewProject_3 "), this, 4, true);
	VSConstFloatValue * pReflectViewProject_4 = MEM_NEW VSConstFloatValue(_T("ReflectViewProject_4 "), this, 4, true);

	//float Dot_1 = dot(WorldPos_One,ReflectViewProject_1);
	VSDot * pDot_1 = MEM_NEW VSDot(_T("Dot_1"), this);
	...
	VSDot * pDot_2 = MEM_NEW VSDot(_T("Dot_2"), this);
	...
	VSDot * pDot_3 = MEM_NEW VSDot(_T("Dot_3"), this);
	...
	VSDot * pDot_4 = MEM_NEW VSDot(_T("Dot_4"), this);
	...
	VSMakeValue * pDot_12 = MEM_NEW VSMakeValue(_T("Dot_12"), this, 2);
	...
	VSMakeValue * pDot_44 = MEM_NEW VSMakeValue(_T("Dot_44"), this, 2);
	...
	VSDiv *pDiv = MEM_NEW VSDiv(_T("Div"), this);
	...
	VSConstFloatValue * pPointFive = MEM_NEW VSConstFloatValue(_T("PointFive"), this, 1, false);
	pPointFive->SetValue(0, 0.5f);

	VSConstFloatValue * pNegPointFive = MEM_NEW VSConstFloatValue(_T("NegPointFive"), this, 1, false);
	pNegPointFive->SetValue(0, -0.5f);

	//float2 FiveNegFive = float2(0.5f,-0.5f);
	VSMakeValue * pFiveNegFive = MEM_NEW VSMakeValue(_T("FiveNegFive"), this, 2);
	...
	VSMakeValue * pFiveFive = MEM_NEW VSMakeValue(_T("FiveFive"), this, 2);
	...
	VSAdd *pAdd = MEM_NEW VSAdd(_T("Add"), this);
	...
	VS2DTexSampler* p2DTexSamplerNode = MEM_NEW VS2DTexSampler(_T("EmissiveTexture"), this);
							   ...
							   VSTexAllStateR * pEmissiveTexture):VSMaterial(ShowName,MUT_OREN_NAYAR)
{
	...
	VS2DTexSampler *pEmissiveNode = MEM_NEW VS2DTexSampler(_T("EmissiveTexture"),this);
	pEmissiveNode->SetTexture(pEmissiveTexture);

	VSConstFloatValue * pRoughnessSquared = MEM_NEW VSConstFloatValue(_T("RoughnessSquared"),this,1,true);
	...
	VSConstFloatValue * pFinalAlpah = MEM_NEW VSConstFloatValue(_T("FinalAlpha"),this,1,false);
										 ...
										 VSTexAllStateR * pSpecularTexture, VSTexAllStateR * pEmissiveTexture,bool bPhoneSpecular):VSMaterial(ShowName,MUT_PHONE)
{
	...
	VS2DTexSampler *pSpecularNode = MEM_NEW VS2DTexSampler(_T("SpecularTexture"),this);
	pSpecularNode->SetTexture(pSpecularTexture);

	VS2DTexSampler *pEmissiveNode = MEM_NEW VS2DTexSampler(_T("EmissiveTexture"),this);
	pEmissiveNode->SetTexture(pEmissiveTexture);

	VSConstFloatValue * pSpecularPow = MEM_NEW VSConstFloatValue(_T("SpecularPow"),this,1,true);
	...
	VSTexAllStateR * pSpecularTexture, VSTexAllStateR * pEmissiveTexture, bool bPhoneSpecular) :VSMaterial(ShowName, MUT_PHONE)
{
	...
	AddPass(MUT_PHONE);
	VSConstFloatValue * pFinalColor = MEM_NEW VSConstFloatValue(_T("FinalColor"), this, 3, false);
...
{
	VSLightDir * pLightDir = MEM_NEW VSLightDir(_T("LightDir"), this);

	VSConstFloatValue * pValue = MEM_NEW VSConstFloatValue(_T("ValueTest"), this, 1, false);
	pValue->SetValue(0, -1.0f);

	VSMul * pMul1 = MEM_NEW VSMul(_T("Mul1"), this);

	pMul1->GetInputNode(0)->Connection(pLightDir->GetOutputNode(0));
	pMul1->GetInputNode(1)->Connection(pValue->GetOutputNode(0));

	VSWorldNormal * pWorldNormal = MEM_NEW VSWorldNormal(_T("WorldNormal"), this);

	VSDot * pDot = MEM_NEW VSDot(_T("Dot"), this);

	pDot->GetInputNode(0)->Connection(pWorldNormal->GetOutputNode(0));
	pDot->GetInputNode(1)->Connection(pMul1->GetOutputNode(0));

	VSSaturate *pSaturate = MEM_NEW VSSaturate(_T("Saturate"), this);
...
:VSMaterial(ShowName, MUT_POSTEFFECT)
{
	...
	VSColorBuffer* pColorBuffer = MEM_NEW VSColorBuffer(_T("ColorBuffer"), this);
	VSConstFloatValue * pMulColor = MEM_NEW VSConstFloatValue(_T("Color"), this, 4, false);
	...
	VSTexAllStateR * pSpecularTexture, VSTexAllStateR * pEmissiveTexture, bool bPhoneSpecular) :VSMaterial(ShowName, MUT_PHONE)
{
	...
	VSConstFloatValue * pExtendLength = MEM_NEW VSConstFloatValue(_T("ExtendLength"), this, 1, true);
	pExtendLength->SetValue(0, 10.0f);

	VSMul * pWorldNormal_Mul_ExtendLength = MEM_NEW VSMul(_T("WorldNormal_Mul_ExtendLength"), this);
	...
	:VSMaterial(ShowName)
{
	...
	VS2DTexSampler * pWorldDisplacementNode = MEM_NEW VS2DTexSampler(_T("WorldDisplacementTexture"), this);
	...
	VSConstFloatValue * pDetaDistance = MEM_NEW VSConstFloatValue(_T("MaxDistance"), this, 1, false);
	pDetaDistance->SetValue(0, -2000.0f);

	VSConstFloatValue * pMinDistance = MEM_NEW VSConstFloatValue(_T("MinDistance"), this, 1, false);
	pMinDistance->SetValue(0, 2500.0f);

	VSCameraWorldPos * pCameraWorldPos = MEM_NEW VSCameraWorldPos(_T("CameraWorldPos"), this);
	VSWorldPos * pWorldPos = MEM_NEW VSWorldPos(_T("WorldPos"), this);
	VSSub * pSub = MEM_NEW VSSub(_T("Sub"), this);
	...
	VSConstFloatValue * pDisplacementScalar = MEM_NEW VSConstFloatValue(_T("DisplacementScalar"), this, 1, false);
	pDisplacementScalar->SetValue(0, 100.0f);

	VSMul * pMulDisplacementScalar = MEM_NEW VSMul(_T("MulDisplacementScalar"), this);

...
using namespace zq;
uint32 VSShaderStringFactory::ms_ShaderValueIndex = 0;
...
uint32 VSShaderStringFactory::ms_uiCreateVShaderNum = 0;
uint32 VSShaderStringFactory::ms_uiCreatePShaderNum = 0;
uint32 VSShaderStringFactory::ms_uiCreateGShaderNum = 0;
uint32 VSShaderStringFactory::ms_uiCreateHShaderNum = 0;
uint32 VSShaderStringFactory::ms_uiCreateDShaderNum = 0;
...
uint32 VSShaderStringFactory::ms_uiShareSamplerRegisterID[];
void VSShaderStringFactory::ClearAllString()
{
	for(uint32 i = 0 ; i < UV_LEVEL; i++)
...
void VSShaderStringFactory::AddShaderSamplerRegisterID(VSShader * pShader,uint32 uiTexRegisterID, uint32 ShareSamplerType)
{
	if (pShader->m_uiShareSamplerRegister.GetNum() == 0)
	{
		uint32 CurMaxShareSamplerRegisterID = GetMaxShareSamplerRegisterID();
	...
	{
		uint32 CurMaxShareSamplerRegisterID = pShader->GetMaxShareSampler();
		pShader->AddShareSampler(CurMaxShareSamplerRegisterID + 1, ShareSamplerType);
	}
}
uint32 VSShaderStringFactory::GetMaxShareSamplerRegisterID()
{
	uint32 Min = INVALID_SIMPLAR_REGISTER;
	for (uint32 i = 0; i < VSEngineFlag::ms_uiShaderSamplerNum; i++)
	{
...
bool VSShaderStringFactory::CreateVShaderString(VSVShader * pVShader,MaterialShaderPara &MSPara,uint32 uiShaderID,VSString & VShaderString
#if _DEBUG 
...
bool VSShaderStringFactory::CreatePShaderString(VSPShader * pPShader,MaterialShaderPara &MSPara,uint32 uiShaderID,VSString & PShaderString
#if _DEBUG 
												,VSString &OutShaderFileName)
#else
												)
...
bool VSShaderStringFactory::CreateGShaderString(VSGShader * pGShader, MaterialShaderPara &MSPara,uint32 uiShaderID, VSString & GShaderString
#if _DEBUG 
	, VSString &OutShaderFileName)
#else
	)
...
bool VSShaderStringFactory::CreateHShaderString(VSHShader * pHShader, MaterialShaderPara &MSPara, uint32 uiShaderID, VSString & HShaderString
#if _DEBUG 
	, VSString &OutShaderFileName)
#else
	)
...
bool VSShaderStringFactory::CreateDShaderString(VSDShader * pDShader, MaterialShaderPara &MSPara, uint32 uiShaderID, VSString & DShaderString
#if _DEBUG 
	, VSString &OutShaderFileName)
#else
	)
...
#if _DEBUG 
void VSShaderStringFactory::OutPutShaderLog(MaterialShaderPara &MSPara, uint32 uiShaderID,
	const VSString & ShaderCode, VSString &OutShaderTextName)
{
    ...
    if (ShaderText.Open(OutShaderTextName.GetBuffer()))
    {

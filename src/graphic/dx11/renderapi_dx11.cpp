...
void VSDX11Renderer::DrawDynamicScreenData(void * pScreenBuffer, unsigned int uiVertexNum, unsigned int uiVertexStride, uint16 * pIndexBuffer, unsigned int uiIndexNum)
{
	struct ScreenData
	{
		void* pScreenBuffer;
	...
	if (VSResourceManager::ms_bRenderThread)
	{
		...
		unsigned int uiIndexLen = sizeof(uint16)* uiIndexNum;
		Screen.pIndexBuffer = (uint16 *)VSRenderThreadSys::ms_pRenderThreadSys->Assign(uiIndexLen);
	...
	ENQUEUE_UNIQUE_RENDER_COMMAND_THREEPARAMETER(VSDx11DrawScreenEXCommand,
		...
		unsigned int IndexBytes = Screen.uiIndexNum * sizeof(uint16);
...
void VSDX11Renderer::OnLoadVShaderFromString(VSVShader * pVShaderProgram, VSVProgramID *pVProgramID)
{
	...
	else if (pVShaderProgram->GetFileName().GetLength())
	{
        ...
        std::string s = Path.GetBuffer();
        std::wstring ws = StringToWString(s);
		hResult = D3DCompileFromFile(ws.c_str(), pMacro, D3D_COMPILE_STANDARD_FILE_INCLUDE,/*ms_pDx11IncludeShader, */pVShaderProgram->GetMainFunName().GetBuffer(),
			ms_cVertexShaderProgramVersion, Flags, 0, &pCode, &pErrors);
	}
	else
		VSMAC_ASSERT(0);
	...
	if (!pVShaderProgram->HasBindResource())
	{
		for (unsigned int uiSamplerIndex = 0; uiSamplerIndex < Desc.BoundResources; uiSamplerIndex++)
		{
			...
			{				
				...
				{
					VSUserBuffer * pUserBuffer = MEM_NEW VSUserBuffer(resDesc.Name, VSDataBuffer::DT_MAXNUM,resDesc.BindPoint,resDesc.BindCount);
				...
				else
				{
					...
					VSUserSampler * pUerSampler = MEM_NEW VSUserSampler(resDesc.Name, uiType, resDesc.BindPoint, resDesc.BindCount);
			...
			{
				VSUserBuffer * pUserBuffer = MEM_NEW VSUserBuffer(resDesc.Name, VSDataBuffer::DT_UBYTE, resDesc.BindPoint, resDesc.BindCount);
				pVShaderProgram->m_pUserBuffer.AddElement(pUserBuffer);
			}
			else if (resDesc.Type == D3D_SIT_STRUCTURED)
			{
				VSUserBuffer * pUserBuffer = MEM_NEW VSUserBuffer(resDesc.Name, VSDataBuffer::DT_STRUCT,resDesc.BindPoint, resDesc.BindCount);
				pVShaderProgram->m_pUserBuffer.AddElement(pUserBuffer);
			}
			else if (resDesc.Type == D3D_SIT_CBUFFER)
			{
				...
				for (unsigned int ValueID = 0; ValueID < cbDesc.Variables; ValueID++)
				{
					...
					VSUserConstant * pUserConstant =
						MEM_NEW VSUserConstant(varDesc.Name,varDesc.Size, varDesc.StartOffset, RegisterNum, uiValueType);
...
void VSDX11Renderer::OnLoadGShaderFromString(VSGShader* pGShaderProgram, VSGProgramID *pGProgramID)
{
	...
	else if (pGShaderProgram->GetFileName().GetLength())
	{
        ...
        std::string s = Path.GetBuffer();
        std::wstring ws = StringToWString(s);
		hResult = D3DCompileFromFile(ws.c_str(), pMacro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
            /*ms_pDx11IncludeShader,*/ pGShaderProgram->GetMainFunName().GetBuffer(), ms_cGeometryShaderProgramVersion, Flags, 0, &pCode, &pErrors);
	}
	else
		VSMAC_ASSERT(0);
...
void VSDX11Renderer::OnLoadHShaderFromString(VSHShader* pHShaderProgram, VSHProgramID *pHProgramID)
{
	...
	else if (pHShaderProgram->GetFileName().GetLength())
	{
        ...
        std::string s = Path.GetBuffer();
        std::wstring ws = StringToWString(s);
		hResult = D3DCompileFromFile(ws.c_str(), pMacro, D3D_COMPILE_STANDARD_FILE_INCLUDE,/*ms_pDx11IncludeShader, */pHShaderProgram->GetMainFunName().GetBuffer(),
			ms_cHullShaderProgramVersion, Flags, 0, &pCode, &pErrors);
	}
	else
		VSMAC_ASSERT(0);
...
void VSDX11Renderer::OnLoadDShaderFromString(VSDShader* pDShaderProgram, VSDProgramID *pDProgramID)
{
	...
	else if (pDShaderProgram->GetFileName().GetLength())
	{
        ...
        std::string s = Path.GetBuffer();
        std::wstring ws = StringToWString(s);
		hResult = D3DCompileFromFile(ws.c_str(), pMacro, D3D_COMPILE_STANDARD_FILE_INCLUDE,/*ms_pDx11IncludeShader, */pDShaderProgram->GetMainFunName().GetBuffer(),
			ms_cDomainShaderProgramVersion, Flags, 0, &pCode, &pErrors);
	}
	else
		VSMAC_ASSERT(0);
...
void VSDX11Renderer::OnLoadCShaderFromString(VSCShader* pCShaderProgram, VSCProgramID *pCProgramID)
{
	...
	else if (pCShaderProgram->GetFileName().GetLength())
	{
        ...
        std::string s = Path.GetBuffer();
        std::wstring ws = StringToWString(s);
		hResult = D3DCompileFromFile(ws.c_str(), pMacro, D3D_COMPILE_STANDARD_FILE_INCLUDE,/*ms_pDx11IncludeShader,*/ pCShaderProgram->GetMainFunName().GetBuffer(),
			ms_cComputerShaderProgramVersion, Flags, 0,  &pCode, &pErrors);
	}
	else
		VSMAC_ASSERT(0);
...
void VSDX11Renderer::OnLoadPShaderFromString(VSPShader* pPShaderProgram, VSPProgramID *pPProgramID)
{
	...
	else if (pPShaderProgram->GetFileName().GetLength())
	{
        ...
        std::string s = Path.GetBuffer();
        std::wstring ws = StringToWString(s);
		hResult = D3DCompileFromFile(ws.c_str(), pMacro, D3D_COMPILE_STANDARD_FILE_INCLUDE,/*ms_pDx11IncludeShader, */pPShaderProgram->GetMainFunName().GetBuffer(),
			ms_cPixelShaderProgramVersion, Flags, 0, &pCode, &pErrors);
	}
	else
		VSMAC_ASSERT(0);
...
void VSDX11Renderer::OnLoadVertexBufferEx(D3D11_BUFFER_DESC& desc,VSVertexBuffer * pVBuffer, VSVBufferID * pVBufferID)
{
		...
		VSOnLoadVertexBufferExPara, OnLoadVertexBufferExPara, OnLoadVertexBufferExPara,
	{
		...
		unsigned char * pTempBufferUsed = MEM_NEW unsigned char[uiTotalSize];
		VSDataBuffer * pData;

...
#include "graphic/dx11/resource_dx11.h"
...
float VSDX11Renderer::ms_fHSMaxTessFactor = 15.0f;
							  ...
							  ChildWindowInfo * pChildWindow,int uiNumChildWindow)
{
	...
	{
		m_pChildWindowInfo = MEM_NEW ChildWindowInfo[uiNumChildWindow];
	...
	if (m_hMainWindow != NULL)
	{
		...
		{
			m_pChain = MEM_NEW IDXGISwapChain*[m_iNumChildWindow];
			m_pChainRenderTargetView = MEM_NEW ID3D11RenderTargetView*[m_iNumChildWindow];
			m_pChainDepthStencil = MEM_NEW ID3D11Texture2D*[m_iNumChildWindow];
			m_pChainDepthStencilView = MEM_NEW ID3D11DepthStencilView*[m_iNumChildWindow];
	...
	SetDefaultValue();
	memset(m_uiShaderBuffer, 0, MAX_SHADER_BUFFER);
	...
	IndexBufferDesc.ByteWidth = sizeof(uint16) * 6 * uiFontNum;
...
VSDX11Renderer::~VSDX11Renderer()
{
	...
	SAFE_DELETEA(m_pChain);
	SAFE_DELETEA(m_pChainRenderTargetView);
	SAFE_DELETEA(m_pChainDepthStencil);
	SAFE_DELETEA(m_pChainDepthStencilView);
...
bool VSDX11Renderer::InitialDefaultState()
{
	...
	for (unsigned int nAdapter = 0; nAdapter < ms_uiAdapterNum; nAdapter++)
	{
		...
		DXGI_MODE_DESC* displayModes = MEM_NEW DXGI_MODE_DESC[DisplayModeNumA8R8G8B8];
	...
	if (!IncludeShaderFile.Open(IncludeShaderPath.GetBuffer(),File::OM_RB))
	{
		return false;
	}
	...
	ms_pDx11IncludeShader = MEM_NEW VSDx11ShaderInclude();
	VSMAC_ASSERT(ms_pDx11IncludeShader)
	return 1;
}
bool VSDX11Renderer::TerminalDefaultState()
{
	SAFE_DELETE(ms_pDx11IncludeShader);
	VSMAC_RELEASE(ms_pDXGI);
	return 1;
}
	...
	int iHeight)
{
	...
	if (pMaterial)
	{
		...
		float ScaleX = (iWidth * 1.0f) / m_uiCurRTWidth;
		float ScaleY = (iHeight * 1.0f) / m_uiCurRTHeight;

		float StartX = (iXPos * 1.0f) / m_uiCurRTWidth;
		float StartY = (iYPos * 1.0f) / m_uiCurRTHeight;

		Vector3W PositionInfoData(StartX, StartY, ScaleX, ScaleY);
...
bool VSDX11Renderer::SetViewPort(VSViewPort *pViewPort)
{
	...
	else
	{
		View.TopLeftX = 0;
		View.TopLeftY = 0;
		View.Width = (float)m_uiCurRTWidth;
		View.Height = (float)m_uiCurRTHeight;
		View.MinDepth = 0.0f;
		View.MaxDepth = 1.0f;
...
{ 
	...
	pVBufferFormatID = MEM_NEW VSVBufferFormatID;
	if (!pVBufferFormatID)
		return 0;
...
bool VSDX11Renderer::DrawScreen(SCREEN_QUAD_TYPE * pScreenBuffer, unsigned int uiVertexNum,uint16 * pIndexBuffer, unsigned int uiIndexNum)
{
...
bool VSDX11Renderer::DrawScreenFont(SCREEN_FONT_TYPE * pScreenFontBuffer, unsigned int uiVertexNum,
	uint16 * pIndexBuffer, unsigned int uiIndexNum)
{
...
void VSDX11Renderer::SetClipPlane(VSArray<Plane> &Plane, bool bEnable){ VSMAC_ASSERT(0); return; }
...
D3D_SHADER_MACRO * VSDX11Renderer::GetDefine(VSMap<VSString, VSString> & Define)
{
	if (Define.GetNum())
	{
		D3D_SHADER_MACRO * pMacro = MEM_NEW D3D_SHADER_MACRO[Define.GetNum() + 1];
...
{ 
	...
	pVProgramID = MEM_NEW VSVProgramID;
	if (!pVProgramID)
		return 0;
	pID = pVProgramID;
...
bool VSDX11Renderer::OnLoadGShaderProgram(VSGShader * pGShaderProgram, VSResourceIdentifier *&pID)
{
	...
	pGProgramID = MEM_NEW VSGProgramID;
	if (!pGProgramID)
		return 0;
	pID = pGProgramID;
...
bool VSDX11Renderer::OnLoadDShaderProgram(VSDShader * pDShaderProgram, VSResourceIdentifier *&pID)
{
	...
	pDProgramID = MEM_NEW VSDProgramID;
	if (!pDProgramID)
		return 0;
	pID = pDProgramID;
...
bool VSDX11Renderer::OnLoadHShaderProgram(VSHShader * pHShaderProgram, VSResourceIdentifier *&pID)
{
	...
	pHProgramID = MEM_NEW VSHProgramID;
	if (!pHProgramID)
		return 0;
	pID = pHProgramID;
...
{ 
	...
	pPProgramID = MEM_NEW VSPProgramID;
	if (!pPProgramID)
		return 0;
	pID = pPProgramID;
...
bool VSDX11Renderer::OnLoadCShaderProgram(VSCShader * pCShaderProgram, VSResourceIdentifier *&pID)
{
	...
	pCProgramID = MEM_NEW VSCProgramID;
	if (!pCProgramID)
		return 0;
	pID = pCProgramID;
...
{ 
	...
	pTextureID = MEM_NEW VSTextureID;
	if (!pTextureID)
		return 0;
...
{ 
	...
	pDepthStencilID = MEM_NEW VSDepthStencilID;
	if (!pDepthStencilID)
		return 0;	
	pID = pDepthStencilID;
...
{ 
	...
	VSRenderTargetID *pRenderTargetID = MEM_NEW VSRenderTargetID;
	pID = pRenderTargetID;
...
bool VSDX11Renderer::OnLoadUnorderAccess(VSBufferUnorderAccess  * pUnorderAccess, VSResourceIdentifier *&pID)
{
	...
	VSUnorderAccessID *pUnorderAccessID = MEM_NEW VSUnorderAccessID;
...
bool VSDX11Renderer::OnLoadVBufferData(VSVertexBuffer * pVBuffer, VSResourceIdentifier *&pID)
{
	...
	pVBufferID = MEM_NEW VSVBufferID;
	if (!pVBufferID)
		return 0;
	pID = pVBufferID;
...
{
	VSSBufferID * pSBufferID = NULL;
	pSBufferID = MEM_NEW VSSBufferID;
	if (!pSBufferID)
		return 0;
	pID = pSBufferID;
...
{ 
	...
	pIBufferID = MEM_NEW VSIBufferID;
	if (!pIBufferID)
		return 0;
	pID = pIBufferID;
...
{ 
	...
	pBlendStateID = MEM_NEW VSBlendStateID;
	if (!pBlendStateID)
		return 0;
	pID = pBlendStateID;
...
{ 
	...
	pDepthStenciStateID = MEM_NEW VSDepthStencilStateID;
...
{ 
	...
	pRasterStateID = MEM_NEW VSRasterStateID;
	if (!pRasterStateID)
		return 0;
	pID = pRasterStateID;
...
{ 
	...
	pSamplerStateID = MEM_NEW VSSamplerStateID;
	if (!pSamplerStateID)
		return 0;
	pID = pSamplerStateID;

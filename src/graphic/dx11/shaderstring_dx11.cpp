...
#include "graphic/dx11/renderer_dx11.h"
#include "graphic/material/shaderfunction/texsampler2d.h"
#include "graphic/material/shaderfunction/texsampler1d.h"
	...
	VSString & OutString)
{
	...
	unsigned int uiAnimSize = sizeof(ANIM_INSTANCE_SHADER_DATA) / sizeof(float) * MAX_SIMPLE_INSTANCE_ANIM;
															 ...
															 VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_LightShadowMatrix,sizeof(float)* 16, 0, 4);
									 ...
									 VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_LightWorldPos,sizeof(float) * 3,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
									  ...
									  VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_LightWorldDirection,sizeof(float) * 3,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
												 ...
												 VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_ViewMatrix,sizeof(float) * 16,0,4);
	pShader->m_pUserConstant.AddElement(pUserConstant);
}
	...
	VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_ProjectMatrix,sizeof(float)* 16, 0, 4);
	pShader->m_pUserConstant.AddElement(pUserConstant);
															 ...
															 VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(VSShaderStringFactory::ms_ViewProjectMatrix,sizeof(float) * 16,0,4);
	pShader->m_pUserConstant.AddElement(pUserConstant);
	...
	VSString & OutString)
{
	if (pGeometry->HasMorphTarget())
	{
		...
		VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_MorphData, sizeof(unsigned int) * 4, 0, 1,VSUserConstant::VT_INT);
		pShader->m_pUserConstant.AddElement(pUserConstant);
		...
		pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_MorphValue, sizeof(float) * 4 * uiRegisterNum, 0, uiRegisterNum, VSUserConstant::VT_FLOAT);
		pShader->m_pUserConstant.AddElement(pUserConstant);
	}
}
	...
	VSString & OutString)
{
	...
	VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_VSAnimDataResource, VSTexture::TT_2D, uiTexRegisterID, 1);
	...
	VSString & OutString)
{
	...
	VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_VSMorphVDataResource, VSTexture::TT_2D, uiTexRegisterID, 1);
	pShader->m_pUserSampler.AddElement(pSampler);
	uiTexRegisterID++;
	...
	if (pVBuffer->HaveNormalInfo(0))
	{
		...
		VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_VSMorphNDataResource, VSTexture::TT_2D, uiTexRegisterID, 1);
		pShader->m_pUserSampler.AddElement(pSampler);
		uiTexRegisterID++;
	}
	...
	if (pVBuffer->HaveTangentInfo())
	{
		...
		VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_VSMorphTDataResource, VSTexture::TT_2D, uiTexRegisterID, 1);
		pShader->m_pUserSampler.AddElement(pSampler);
		uiTexRegisterID++;
		if (pVBuffer->HaveBinormalInfo())
		{
			...
			VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_VSMorphBDataResource, VSTexture::TT_2D, uiTexRegisterID, 1);
			pShader->m_pUserSampler.AddElement(pSampler);
			uiTexRegisterID++;
		}
	}
							...
							VSString & OutString)
{
	...
	if (pInstanceGeometry && pVBuffer->HaveBlendIndicesInfo(1))
	{
		...
		VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_AnimAllInfo, sizeof(unsigned int) * 4, 0, 1, VSUserConstant::VT_INT);
		pShader->m_pUserConstant.AddElement(pUserConstant);
	}
	else if (pVBuffer->HaveBlendIndicesInfo(0))
	{
		...
		VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_BoneMatrix,sizeof(float) * 4 * uiBoneNum * 3,0,uiBoneNum * 3);
		pShader->m_pUserConstant.AddElement(pUserConstant);
	}
}
												  ...
												  VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_WorldMatrix,sizeof(float) * 16,0,4);
	pShader->m_pUserConstant.AddElement(pUserConstant);
													 ...
													 VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_CameraWorldPos,sizeof(float) * 3,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
													 ...
													 VSString & OutString)
{
	if (uiLightNum > 0)
	{
		...
		VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_LightName[VSLight::LT_DIRECTION],sizeof(float) * 84 * uiLightNum,0,21 * uiLightNum);
		pShader->m_pUserConstant.AddElement(pUserConstant);
												 ...
												 VSString & OutString)
{
	if (uiLightNum > 0)
	{
		...
		VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_LightName[VSLight::LT_POINT],sizeof(float) * 52 * uiLightNum,0,13 * uiLightNum);
		pShader->m_pUserConstant.AddElement(pUserConstant);
												...
												VSString & OutString)
{
	if (uiLightNum > 0)
	{
		...
		VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_LightName[VSLight::LT_SPOT],sizeof(float) * 56 * uiLightNum,0,14 * uiLightNum);
		pShader->m_pUserConstant.AddElement(pUserConstant);
...
void VSDX11Renderer::CreateUserConstantLightFunction(VSPShader* pPShader,VSArray<VSLight*> & LightArray,unsigned int &ID,VSString & OutString)
{
	...
	for (unsigned int i = 0 ; i < LightArray.GetNum() ; i++)
	{
		...
		if (pLocalLight && pLocalLight->HaveLightFun())
		{
			...
			VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSConstantLightFunResource[uiLightFunction],VSTexture::TT_2D,ID,1);
		...
		if (pLocalLight && pLocalLight->GetCastShadow())
		{
			...
			if (pLocalLight->GetLightType() == VSLight::LT_POINT)
			{
				if(((VSPointLight*)pLocalLight)->GetShadowType() == VSPointLight::ST_CUB)
				{
					...
					VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSConstantShadowResource[uiShadowNum],VSTexture::TT_CUBE,ID,1);
					pPShader->m_pUserSampler.AddElement(pSampler);
					uiShadowNum++;
					ID++;
				...
				if(((VSPointLight*)pLocalLight)->GetShadowType() == VSPointLight::ST_VOLUME)
				{
					...
					VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSConstantShadowResource[uiShadowNum],VSTexture::TT_2D,ID,1);
					pPShader->m_pUserSampler.AddElement(pSampler);
					uiShadowNum++;
					ID++;
				...
				if(((VSPointLight*)pLocalLight)->GetShadowType() == VSPointLight::ST_DUAL_PARABOLOID)
				{
					...
					VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSConstantShadowResource[uiShadowNum],VSTexture::TT_2D,ID,2);
					pPShader->m_pUserSampler.AddElement(pSampler);
					uiShadowNum++;
					ID++;
					ID++;
			...
			else if (pLocalLight->GetLightType() == VSLight::LT_DIRECTION)
			{
				if(((VSDirectionLight*)pLocalLight)->GetShadowType() == VSDirectionLight::ST_CSM)
				{
					...
					VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSConstantShadowResource[uiShadowNum],VSTexture::TT_2D,ID,3);
					pPShader->m_pUserSampler.AddElement(pSampler);
					uiShadowNum++;
					ID++;
					ID++;
...
void VSDX11Renderer::CreateGBufferSampler(VSPShader* pPShader, MaterialShaderPara &MSPara, unsigned int &uiTexRegisterID, VSString & OutString)
{
	...
	if (pMaterial->GetPostEffectShaderFunction())
	{
		...
		VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSColorBufferResource, VSTexture::TT_2D, uiTexRegisterID, 1);
		pPShader->m_pUserSampler.AddElement(pSampler);
		uiTexRegisterID++;
		...
		pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSDepthNormalBufferResource, VSTexture::TT_2D, uiTexRegisterID, 1);
		pPShader->m_pUserSampler.AddElement(pSampler);
		uiTexRegisterID++;
	}
}
void VSDX11Renderer::CreateLightModleLookUpTable(VSPShader* pPShader,MaterialShaderPara &MSPara, unsigned int &uiTexRegisterID,VSString & OutString)
{
	...
	if (pMaterial->GetShaderMainFunction(MSPara.uiPassId)->GetSMType() == VSShaderMainFunction::SM_OREN_NAYAR)
	{
		...
		if (pOrenNayarShaderFunction->UseLookUpTable())
		{
			...
			VSUserSampler * pSampler = MEM_NEW VSUserSampler(*VSShaderStringFactory::ms_PSOrenNayarLookUpTableResource,VSTexture::TT_2D,uiTexRegisterID,1);
			pPShader->m_pUserSampler.AddElement(pSampler);
			uiTexRegisterID++;
		}
	}
							...
							VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_FarZ,sizeof(float) * 1,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
									   ...
									   VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_SkyLightUpColor,sizeof(float) * 4,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
										 ...
										 VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_SkyLightDownColor,sizeof(float) * 4,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
									   ...
									   VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_PointLightRange,sizeof(float) * 1,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
							   ...
							   VSString & OutString)
{
	...
	VSUserConstant * pUserConstant = MEM_NEW VSUserConstant(*VSShaderStringFactory::ms_InvRTWidth,sizeof(float) * 1,0,1);
	pShader->m_pUserConstant.AddElement(pUserConstant);
...
void VSDX11Renderer::GreaterZeroPow(const VSString & Value,float Exp,VSString &OutString) const
{
...
void VSDX11Renderer::SaturatePow(const VSString & Value,float Exp,VSString &OutString) const
{
	VSString SExp = RealToString(Exp);

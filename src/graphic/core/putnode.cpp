...
END_ADD_PROPERTY
VSPutNode::VSPutNode(uint32 uiValueType,const VSUsedName & NodeName,VSObject * pObject)
{
	m_pOwner = pObject;
	m_NodeName = NodeName;
...
VSInputNode::VSInputNode(uint32 uiValueType,const VSUsedName &NodeName,VSObject * pObject)
...
VSOutputNode::VSOutputNode(uint32 uiValueType,const VSUsedName &NodeName,VSObject * pObject)
			:VSPutNode(uiValueType,NodeName,pObject)
{
...
bool VSOutputNode::CheckIsConnection(VSInputNode * pInputNode)const
{
	if(!pInputNode)
		return 0;
	for(uint32 i = 0 ; i < m_pInputLink.GetNum() ; i++)
	{
...
void VSInputNode::UnConnection()
{
	if(!m_pOutputLink)
		return;
	for(uint32 i = 0 ; i < m_pOutputLink->m_pInputLink.GetNum() ; i++)
	{

...
VSName::VSName(const char * pChar,uint32 uiID)
{
	VSMAC_ASSERT(pChar);
	m_uiID = uiID;
	m_String.Clear();
...
VSName::VSName(const VSString & VSString,uint32 uiID)
{
	m_uiID = uiID;
	m_String.Clear();
	m_String = VSString;
...
VSUsedName::VSUsedName(const char * pChar)
{
	m_pName = NULL;

	m_pName = VSResourceManager::CreateName(pChar);
}
VSUsedName::VSUsedName(const VSString & VSString)
{
	m_pName = NULL;
	m_pName = VSResourceManager::CreateName(VSString);
}
void VSUsedName::operator =(const VSString &VSString)
{
	m_pName = NULL;
	m_pName = VSResourceManager::CreateName(VSString);
		
}
void VSUsedName::operator =(const char *pChar)
{
	m_pName = NULL;
...
void VSUsedName::Archive(VSStream & Stream)
{
	...
	else if (Stream.GetStreamFlag() == VSStream::AT_SIZE)
	{
		...
		else
		{
			...
			Stream.AddBufferSize(VSStream::GetStrDistUse(""));
		}
	}
}
...
bool operator ==(const VSUsedName &Name,const VSString & VSString)
{
	if (!Name.m_pName)
	{
		return 0;
	}
	return Name.m_pName->GetString() == VSString;
}
...
bool operator ==(const VSUsedName &Name,const char * pChar)
{

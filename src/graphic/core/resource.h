...
class VSStream;
class GRAPHIC_API VSResourceInterface
{
public:
	VSResourceInterface();
...
public:\
constexpr static char* ms_FileSuffix = #FileSuffix;\
constexpr static char* ms_PointFileSuffix = "."#FileSuffix;\
constexpr static char* ms_ResourcePath = #DefaultPath"/";
...
public:\
constexpr static uint32 ms_uiResourceAbility = ResourceAbility;\
constexpr static uint32 ms_ResourceType = ResourceType;\
static uint32 GetResourceType()\
{\
...
class GRAPHIC_API VSResource : public VSResourceInterface
{
public:
	enum	//Resource Type
	{
...
constexpr static char * ms_CachePath = _T(#CachePath)_T("/");\
...
class GRAPHIC_API VSResourceProxyBase : public VSReference , public MemObject , public VSStreamingType
{
public:
	VSResourceProxyBase();
	virtual ~VSResourceProxyBase() = 0;

	virtual uint32 GetResourceAbility() = 0;
...
class GRAPHIC_API VSCacheResource : public VSObject, public VSResourceInterface
{
public:
	//RTTI
	DECLARE_RTTI;
...
public:
	...
	virtual uint32 GetResourceAbility();
	virtual uint32 GetResourceType();
...
{
	VSResourceProxy<T> * pProxy = MEM_NEW VSResourceProxy<T>();
...
template<class T>
uint32 VSResourceProxy<T>::GetResourceAbility()
{
	return T::ms_uiResourceAbility;
}

template<class T>
uint32 VSResourceProxy<T>::GetCurStreamLevel() const
{
...
template<class T>
uint32 VSResourceProxy<T>::GetResourceType()
{
...
{
public:
	VSResourceControl(uint32 uiGCMaxTimeCount);
	...
	{
		for (uint32 i = 0; i < GetResourceRtti().GetNum(); i++)
		{
...
{
public:
	VSResourceArrayControl(uint32 uiGCMaxTimeCount = 5000);
	...
	T  GetResource(uint32 i);
	void ClearTimeCount(uint32 i);
	uint32 GetResourceNum();
protected:
	VSArrayOrder<T> m_ResourceArray;
};

template<class T>
VSResourceArrayControl<T>::VSResourceArrayControl(uint32 uiGCMaxTimeCount) : VSResourceControl(uiGCMaxTimeCount)
{
...
template<class T>
T VSResourceArrayControl<T>::GetResource(uint32 i)
{
...
template<class T>
uint32 VSResourceArrayControl<T>::GetResourceNum()
{
	return m_ResourceArray.GetNum();
}

template<class T>
void VSResourceArrayControl<T>::ClearTimeCount(uint32 i)
{
	m_TimeCount[i] = 0;
}
...
{
	uint32 i = m_ResourceArray.FindElement(R);
	if (i == m_ResourceArray.GetNum())
	{
		return false;
	}
...
{
protected:
	VSMap<KEY, VALUE> m_Resource;
public:
	VSResourceSet(uint32 uiGCMaxTimeCount = 5000);
	~VSResourceSet();
	...
	virtual bool IsReleseAll();
	virtual void ClearAll();
	const MapElement<KEY, VALUE> * GetResource(uint32 i);
	virtual void GCResource();
	uint32 GetResourceIndexByKey(const KEY & Key)const;
};
...
{
public:
	VSProxyResourceSet(uint32 uiGCMaxTimeCount = 5000);
	~VSProxyResourceSet();
	virtual void GCResource();
};

template<class KEY, class VALUE>
uint32 VSResourceSet<KEY, VALUE>::GetResourceIndexByKey(const KEY & Key)const
{
	uint32 i = m_Resource.Find(Key);
	return i;
}
...
template<class KEY, class VALUE>
VSResourceSet<KEY, VALUE>::VSResourceSet(uint32 uiGCMaxTimeCount) : VSResourceControl(uiGCMaxTimeCount)
{
	m_Resource.Clear();
}
...
template<class KEY, class VALUE>
uint32 VSResourceSet<KEY, VALUE>::GetResourceNum()
{
...
template<class KEY, class VALUE>
const MapElement<KEY, VALUE> * VSResourceSet<KEY, VALUE>::GetResource(uint32 i)
{
...
template<class KEY, class VALUE>
VSProxyResourceSet<KEY, VALUE>::VSProxyResourceSet(uint32 uiGCMaxTimeCount) :VSResourceSet<KEY, VALUE>(uiGCMaxTimeCount)
{
}

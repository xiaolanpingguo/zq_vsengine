...
VSMap<VSUsedName,FactoryFunction> VSObject::ms_ClassFactory;
...
uint32 VSFastObjectManager::GetObjectNum()
{
...
VSObject* VSObject::CloneCreateObject(VSObject * pObject)
{
	...
	for (uint32 i = 0 ; i < CloneMap.GetNum() ;i++)
	{
...
VSObject* VSObject::_CloneCreateObject(VSObject * pObject,VSMap<VSObject *,VSObject*>& CloneMap)
{
	...
	if (pObject)
	{
		uint32 uiIndex = CloneMap.Find(pObject);
		...
		if (uiIndex == CloneMap.GetNum())
		{
			...
			for (uint32 i = 0 ; i < SrcRtti.GetPropertyNum() ; i++)
			{
...
void VSObject::_CloneObject(VSObject * pObjectSrc,VSObject * pObjectDest,VSMap<VSObject *,VSObject*>& CloneMap)
{
	...
	uint32 uiIndex = CloneMap.Find(pObjectSrc);
...
{
	uint32 i = ms_ClassFactory.Find(sRttiName);
...
bool VSObject::Process(VSUsedName & FunName, void * para, void *ret, int32 ParaNum)
{
	VSRtti & t = GetType();
	for (uint32 i = 0; i < t.GetFunctionNum(); i++)
	{

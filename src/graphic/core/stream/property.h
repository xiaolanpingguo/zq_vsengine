...
#include "graphic/core/stream/propertymarc.h"
...
namespace zq{
	...
	public:
		...
		VSProperty(VSRtti & Owner,const VSUsedName & Name,uint32 uiElementOffset,uint32 uiFlag)
			:m_pRttiOwner(&Owner)
		{
			m_Name = Name;
		...
		VSProperty(VSFunction & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag)
		...
		virtual uint32 GetPropertyType()const = 0;
	...
	public:
		...
		VSEnumProperty(VSRtti & Owner,const VSUsedName & Name,const VSUsedName & EnumName,uint32 uiElementOffset,uint32 uiFlag)
			:VSProperty(Owner,Name,uiElementOffset,uiFlag)
		{
			VSMAC_ASSERT(sizeof(T) == 4);
			m_EnumName = EnumName;
		}
		VSEnumProperty(VSFunction & Owner, const VSUsedName & Name, const VSUsedName & EnumName, uint32 uiElementOffset, uint32 uiFlag)
			:VSProperty(Owner, Name, uiElementOffset, uiFlag)
		{
			VSMAC_ASSERT(sizeof(T) == 4);
			m_EnumName = EnumName;
		...
		{
			VSEEnumProperty * pEp = MEM_NEW VSEEnumProperty((unsigned int*)(((const char*)pSrcObj) + m_uiElementOffset),Name,(VSObject*)pSrcObj);
		...
		{
			return MEM_NEW VSEnumProperty();
		}
		VSUsedName & GetEnumName()
		{
			return m_EnumName;
		}
		virtual uint32 GetPropertyType()const
		{
			return PT_ENUM;
		}
	protected:
	...
	{
	public:
		VSDataProperty(VSRtti & Owner,const VSUsedName & Name,uint32 uiElementOffset,uint32 uiDataNum,bool bDynamicCreate)
			:VSProperty(Owner,Name,uiElementOffset,F_SAVE_LOAD_CLONE | F_NO_USE_GC)
		{
			m_uiDataNum = uiDataNum;
			m_bDynamicCreate = bDynamicCreate;
		}
		VSDataProperty(VSFunction & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiDataNum, bool bDynamicCreate)
			:VSProperty(Owner, Name, uiElementOffset, F_SAVE_LOAD_CLONE | F_NO_USE_GC)
		{
			m_uiDataNum = uiDataNum;
			m_bDynamicCreate = bDynamicCreate;
		...
		VSDataProperty(VSRtti & Owner,const VSUsedName & Name,uint32 uiElementOffset,uint32 uiNumElementOffset)
			:VSProperty(Owner, Name, uiElementOffset, F_SAVE_LOAD_CLONE | F_NO_USE_GC)
		{
			m_uiDataNum = 0;
		...
		virtual void CloneData(void* pSrcObj,void* pDestObj,VSMap<VSObject *,VSObject*>& CloneMap)
		{
			...
			else
			{
				...
				*Temp = MEM_NEW T[uiNum];
				VSMemcpy((void *)(*Temp),(void *)SrcValueAddres,uiNum * sizeof(T));
			}
		}
		virtual bool Archive(VSStream &Stream,void* pObj)
		{
			uint32 uiStreamFlag = Stream.GetStreamFlag();
		...
		virtual VSProperty * GetInstance()
		{
			return MEM_NEW VSDataProperty<T,NumType>();
		}
		virtual bool Clone(VSProperty * p)
		{
	...
	public:
		...
		VSValueBaseProperty(VSRtti & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false, T HighValue = T(), T LowValue = T(), T fStep = T())
			:VSProperty(Owner,Name,uiElementOffset,uiFlag)
		{
			m_LowValue = LowValue;
		...
		VSValueBaseProperty(VSFunction & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false, T HighValue = T(), T LowValue = T(), T fStep = T())
			:VSProperty(Owner, Name, uiElementOffset, uiFlag)
		{
			m_LowValue = LowValue;
			m_HightValue = HighValue;
	...
	public:
		...
		VSValueProperty(VSRtti & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false ,T HighValue = T(), T LowValue = T(), T fStep = T())
			:VSValueBaseProperty(Owner, Name, uiElementOffset, uiFlag, Range,HighValue, LowValue, fStep)
		{

		}
		VSValueProperty(VSFunction & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false, T HighValue = T(), T LowValue = T(), T fStep = T())
			:VSValueBaseProperty(Owner, Name, uiElementOffset, uiFlag, Range, HighValue, LowValue, fStep)
		{
		...
		virtual VSProperty * GetInstance()
		{
			return MEM_NEW VSValueProperty<T>();
		}
	};
	...
	public:
		...
		VSArrayProperty(VSRtti & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false, T HighValue = T(), T LowValue = T(), T fStep = T())
			:VSValueBaseProperty(Owner, Name, uiElementOffset, uiFlag, Range, HighValue, LowValue, fStep)
		{

		}
		VSArrayProperty(VSFunction & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false, T HighValue = T(), T LowValue = T(), T fStep = T())
			:VSValueBaseProperty(Owner, Name, uiElementOffset, uiFlag, Range, HighValue, LowValue, fStep)
		{
		...
		inline bool AddElement(void* pObj,uint32 uiIndex,T& pDataSrc)
		{
			GetContainer(pObj).AddElement(pDataSrc);
			return true;
		}
		inline bool Erase(void* pObj,uint32 i)
		{
			GetContainer(pObj).Erase(i);
		}
		virtual bool SetValue(void* pObj,uint32 uiIndex,T& pDataSrc) const
		{
		...
		virtual bool GetValue(void* pObj, uint32 uiIndex, T& pDataDest) const
		{
		...
		virtual VSProperty * GetInstance()
		{
			return MEM_NEW VSArrayProperty<ArrayType,T>();
		}
	};
	...
	{
	public:
		VSMapProperty(VSRtti & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false,VALUE HighValue = VALUE(), VALUE LowValue = VALUE(), VALUE fStep = VALUE())
			:VSValueBaseProperty(Owner, Name, uiElementOffset, uiFlag, Range, HighValue, LowValue, fStep)
		{

		}
		VSMapProperty(VSFunction & Owner, const VSUsedName & Name, uint32 uiElementOffset, uint32 uiFlag, bool Range = false, VALUE HighValue = VALUE(), VALUE LowValue = VALUE(), VALUE fStep = VALUE())
			:VSValueBaseProperty(Owner, Name, uiElementOffset, uiFlag, Range, HighValue, LowValue, fStep)
		{
		...
		inline bool AddElement(void* pObj,uint32 uiIndex,MapElement<KEY,VALUE>& pDataSrc)
		{
			GetContainer(pObj).AddElement(pDataSrc);
			return true;
		}
		...
		virtual bool SetValue(void* pObj,uint32 uiIndex,KEY & Key,VALUE& Value) const
		{
		...
		virtual bool GetValue(void* pObj, uint32 uiIndex, MapElement<KEY,VALUE>& pDataDest) const
		{
			pDataDest = (GetContainer(pObj)[uiIndex]);
			return true;
		}
		...
		virtual VSProperty * GetInstance()
		{
			return MEM_NEW VSMapProperty<MapType,KEY,VALUE>();
		}
	};
	...
	{
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,uint32 NumOffset)
		{

			return MEM_NEW VSDataProperty<T,NumType>(Owner, Name,Offset,NumOffset);
		}
		VSProperty* CreateFunctionProperty(const VSUsedName & Name, VSFunction & Owner, uint32 Offset, uint32 NumOffset)
		{

			return MEM_NEW VSDataProperty<T, NumType>(Owner, Name, Offset, NumOffset);
		}
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,uint32 uiDataNum,bool bDynamicCreate)
		{

			return MEM_NEW VSDataProperty<T,NumType>(Owner, Name,Offset,uiDataNum,bDynamicCreate);
		}
	};
	template<class T>
	struct AutoPropertyCreator
	{
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,uint32 uiFlag)
		{
			...
			return MEM_NEW VSValueProperty<T>(Owner, Name,Offset,uiFlag);
	
			
		}
		VSProperty* CreateFunctionProperty(const VSUsedName & Name, VSFunction & Owner, uint32 Offset, uint32 uiFlag)
		{

			return MEM_NEW VSValueProperty<T>(Owner, Name, Offset, uiFlag);
		}
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,T HighValue ,T LowValue,float fStep,uint32 uiFlag)
		{
			if (!(TIsVSPointerType<T>::Value || TIsVSSmartPointerType<T>::Value || TIsCustomType<T>::Value || TIsVSType<T>::Value))
			{
				uiFlag |= VSProperty::F_NO_USE_GC;
			}
			return MEM_NEW VSValueProperty<T>(Owner, Name, Offset, uiFlag,true,HighValue, LowValue, fStep);
		}
	};

	template<class T,class VSMemManagerClass>
	struct AutoPropertyCreator<VSArray<T,VSMemManagerClass>>
	{
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,uint32 uiFlag)
		{
			...
			return MEM_NEW VSArrayProperty<VSArray<T,VSMemManagerClass>,T>(Owner, Name,Offset,uiFlag);
		...
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,T HighValue ,T LowValue,T fStep,uint32 uiFlag)
		{
			if (!(TIsVSPointerType<T>::Value || TIsVSSmartPointerType<T>::Value || TIsCustomType<T>::Value || TIsVSType<T>::Value || ValueBase<T>::NeedsConstructor))
			{
				uiFlag |= VSProperty::F_NO_USE_GC;
			}
			return MEM_NEW VSArrayProperty<VSArray<T, VSMemManagerClass>, T>(Owner, Name, Offset, uiFlag, HighValue, LowValue, fStep);
	...
	template<class T,class VSMemManagerClass>
	struct AutoPropertyCreator<VSArrayOrder<T,VSMemManagerClass>>
	{
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,uint32 uiFlag)
		{
			...
			return MEM_NEW VSArrayProperty<VSArrayOrder<T,VSMemManagerClass>,T>(Owner, Name,Offset,uiFlag);
		}
		VSProperty* CreateFunctionProperty(const VSUsedName & Name, VSFunction & Owner, uint32 Offset, uint32 uiFlag)
		{
		...
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,T HighValue ,T LowValue,float fStep,uint32 uiFlag)
		{
			...
			return MEM_NEW VSArrayProperty<VSArrayOrder<T, VSMemManagerClass>, T>(Owner, Name, Offset, uiFlag, HighValue, LowValue, fStep);

		}
	};

	template<class KEY,class VALUE,class VSMemManagerClass>
	struct AutoPropertyCreator<VSMapOrder<KEY,VALUE,VSMemManagerClass>>
	{
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,uint32 uiFlag)
		{

			return MEM_NEW VSMapProperty<VSMap<KEY,VALUE,VSMemManagerClass>,KEY,VALUE>(Owner, Name,Offset,uiFlag);
		...
		VSProperty* CreateProperty(const VSUsedName & Name, VSRtti & Owner, uint32 Offset,VALUE HighValue ,VALUE LowValue,float fStep,uint32 uiFlag)
		{

			return MEM_NEW VSMapProperty<VSMap<KEY, VALUE, VSMemManagerClass>, KEY, VALUE>(Owner, Name, Offset, uiFlag, HighValue, LowValue, fStep);
	...
	template<class KEY,class VALUE,class VSMemManagerClass>
	struct AutoPropertyCreator<VSMap<KEY,VALUE,VSMemManagerClass>>
	{
		VSProperty* CreateProperty(const VSUsedName & Name,VSRtti & Owner, uint32 Offset,uint32 uiFlag)
		{
	...
	public:
		...
		template<class ValueType>
		static VSProperty* CreateEnumProperty(ValueType& valueTypeDummyRef,const VSUsedName & Name, const VSUsedName & EnumName,VSRtti & Owner, uint32 Offset,uint32 uiFlag)
		{
			return MEM_NEW VSEnumProperty<ValueType>(Owner, Name, EnumName, Offset, uiFlag | VSProperty::F_NO_USE_GC);
		}
	};
	...
	public:
		...
		VSFunction(VSRtti & Owner, const VSUsedName & Name, uint32 uiFlag)
		:m_pOwner(&Owner)
		{
			m_Name = Name;
			m_uiFlag = uiFlag;
		...
		VSProperty * GetProperty(uint32 i)
		{
		...
		void SetTotalSize(uint32 uiTotalSize)
		{

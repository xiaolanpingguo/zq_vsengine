...
VSString::VSString(const TCHAR * String)
{
	...
	if(String)
	{
		...
		m_pBuffer= MEM_NEW TCHAR[uiLength + 1];
		VSMAC_ASSERT(m_pBuffer);
...
const VSString & VSString::operator +=(const VSString &String)
{
	...
	else
	{
		...
		pBuffer = MEM_NEW TCHAR[uiLength];
...
bool VSString::RemoveSubString(const VSString &String, int iFindNum)
{
	if (GetLength() < String.GetLength())
	{
		return false;
...
void VSString::Remove(unsigned int i, unsigned int j)
{
	if (j > GetLength() - 1)
	{
		j = GetLength() - 1;
	...
	TCHAR * pBuffer = MEM_NEW TCHAR[DeltaLength + 1];
	pBuffer[DeltaLength] = _T('\0');
	if (i > 0)
	{
		VSMemcpy(pBuffer, m_pBuffer, i);
	}
	unsigned int LeftNum = DeltaLength - i;
	if (LeftNum > 0)
	{
		VSMemcpy(pBuffer + i, m_pBuffer + i + uiLength, LeftNum);
	}
	SAFE_DELETEA(m_pBuffer);
	m_pBuffer = pBuffer;
}
bool VSString::StripChars(const VSString &String, const TCHAR * pStripChars)
{
...
bool VSString::ReplaceChars(const VSString &String , const TCHAR * pReplaceChars,TCHAR UseChar)
{
...
float StringToReal(const VSString & String)
{
	float f = 0.0f;
	VSScanf(String.GetBuffer(), "%f", (TCHAR *)&f);
	return f;
}
VSString RealToString(float Value)
{

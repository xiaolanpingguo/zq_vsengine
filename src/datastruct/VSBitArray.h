#pragma once
#include "datastruct/VSContainer.h"
namespace zq
{
	template<class T>
	class VSBit
	...
	public:
		...
		VSBitArray(const VSBitArray & BitArray)
		{
			...
			m_pBuffer = MEM_NEW unsigned int[m_uiSize];
		...
		void Resize( unsigned int uiSize )
		{
			...
			pNewVector = MEM_NEW unsigned int[uiSize];
			
			VSMAC_ASSERT(pNewVector);

			memset(pNewVector,0,uiSize * sizeof(unsigned int));
			unsigned int uiMin;
			if( uiSize < m_uiSize )
				uiMin = uiSize;
			else

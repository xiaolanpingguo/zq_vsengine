...
{
	if(File.Open(filename,File::OM_RB))
		return true;
	return false;
...
int VSMatch::FunIntMatch_D()
{
	...
	TCHAR *num=MEM_NEW TCHAR[p_index-_p_index+1];
...
bool VSMatch::FunIntMatch()
{
	...
	b_index--;
	b_index++;
	if(buffer[b_index]==_T('+') || buffer[b_index]==_T('-') || VSIstalnum(buffer[b_index]))
	{
		num_len++;
	...
	while(1)
	{
		...
		else if(VSIstalnum(buffer[b_index]))
		{
			num_len++;
			continue;
		}
...
bool VSMatch::FunFloMatch()
{
	...
	{ 
		...
		b_index++;
		while(VSIstalnum(buffer[b_index]))
		{
			num_len++;
			b_index++;

...
{
	VSGraphNode * p = MEM_NEW VSGraphNode();
...
VSGraphEdge *VSGraph::CreateEdge(VSGraphNode * pFrom,VSGraphNode * pTo,float fCost)
{
	VSMAC_ASSERT(pFrom && pTo);

	VSGraphEdge * p = MEM_NEW VSGraphEdge();
...
void VSGraph_SearchCommon::ReSort(VSGraphNode * pGraphNode,float fCost)
{
...
void VSGraph_SearchDijkstra::Search()
{
	...
	while(m_Priority.GetNum())
	{
		...
		float fCurCost = m_CostToThisNode[uiIndex].Value; 
		for(;k.IsValid() ; ++k)
		{
			float fNewCost = fCurCost + (*k)->GetCost();
...
float VSGraph_SearchDijkstra::GetCostToTarget()const
{
	if (!m_pTarget)
	{
		return 0.0f;
...
float VSGraph_SearchDijkstra::GetCostToNode(VSGraphNode * pGraphNode)const
{
	if (!pGraphNode)
	{
		return 0.0f;
